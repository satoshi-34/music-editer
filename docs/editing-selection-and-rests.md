...
### 線を少し外したクリックも選択にする（Issue #271）

上の丸めだけだと、選択が成立するのは「その音の線ちょうど ± 0.25 ライン」（100% ズームで約 2.4px）
だけになります。2 度でぶつかる和音（例 `[e/4, f#/4]`）の上の音は狙っても外しやすく、
外すと和音追加になって「押した覚えのない音」が増えてしまいます。

そこで **符頭の X 範囲内のクリックに限り、既存の構成音の ± 0.3 ライン以内なら和音追加より選択を優先**します
（`INNER_KEY_SELECT_MAX_LINES`）。

- 既存音から 0.5 ライン以上離れた位置は従来どおり和音追加です（隣の線／間は狙えます）
- 符頭から横に離れた位置の挿入・和音追加は変わりません
- 固定範囲の外は従来どおり ± 1 ライン（`OUTER_KEY_SELECT_MAX_LINES`, Issue #255）

「押したら選択になるか」の判定は `resolveSelectableKeyIndexAt()` の 1 本だけです。
クリック・ホバーのカーソル形状（`pointer` / `copy`）・再クリック巡回（Issue #264）は
すべてこの関数を呼びます。**判定を別の式で書き写さないでください**（ホバー表示だけ嘘になります）。
...
- 広げるのは**符頭の左右幅ぶんだけ**です（Issue #246）。同じ高さでも符頭から横に離れた位置は
  この音符のヒット領域に入らないので、クリックは隣のパートや小節背景へそのまま届きます。
  ヒット領域の rect は `data-hit-part="fixed"`（固定範囲・セル全幅）と
  `data-hit-part="extension"`（広げたぶん・符頭幅）に分かれています
...
休符をクリックしたときの動きは、**そのとき選んでいるツール**で変わります。

| 選んでいるツール | 休符本体をクリックしたとき |
|---|---|
| 音価ツール（音符側） | **1 クリックで**置換または分割する |
| 休符ツール・調整ツールなど「音符を置かないツール」 | 休符を選択する（Delete・↑/↓・0 キーの対象にできる） |

休符**本体**のクリックはこの 2 つだけで、音符・休符が増えることはありません。
休符の隣に置きたいときは、休符本体の外側（時間枠の広いヒット範囲）をクリックします。

以前は音価ツールでも「1 回目のクリックで選択・2 回目で置換」の 2 段階でしたが、
三連符が主体の曲では音符の 2/3 がこの 2 クリック操作になり入力テンポを大きく削いでいたため、
1 クリックに変えました（Issue #233）。誤クリックは Undo（1 操作＝1 履歴）で戻せます。
...
「音符を置かないツール」では `buildRestEditReplacement()` が `null` を返すので、置換されず選択になります。
つまり **1 クリック化の分岐は、この関数の戻り値だけで決まります**（ツール判定をクリック側に二重に書いていません）。

ホバー中のカーソルも同じ関数を通して決めます。音価ツールで休符本体に乗ったときは、
置けるなら `copy`、置けない（連符内で音価が違う・ツールの音符のほうが長いなど）なら `not-allowed` です。
1 クリックで確定するようになったぶん、押す前に結果が分かることが重要になっています。
...
### クリックした拍まで埋める（Issue #322・複数段譜）

上の補完とは別に、**編集中の小節の中**でも「クリックした拍の手前」を休符で埋めます。

空の声部では挿入位置（`doInsert` の `at`）の比較対象が無く、小節の途中をクリックしても
必ず先頭に入っていました。合同フォーマットで**同じ開始拍の音符は声部・パートをまたいで同じ X に並ぶ**
ことを利用し、他の声部・パートの列からクリックXの拍を逆引きします。

- 拍とXの対応表は描画時に `beatColumnsByMeasureP`（小節番号 → `BeatColumn[]`）へ積みます
- 判定は `utils/beatColumnUtils.ts` の純粋関数（`planLeadingRestFillBeats`）に集約しています
- 働くのは挿入位置がアクティブ声部の末尾のときだけで、既存音符の近くのクリックは従来どおりです
