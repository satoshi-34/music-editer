...
## 段1（本 PR）: 音源に依存しない方法
- **弱い音だけ**にローパスフィルタ（BiquadFilter・Q 0 dB）を **2 段直列**（24dB/oct）で挟み、カットオフを velocity で動かす。
  1 段では「もっと柔らかくならない？」（ユーザー 2026-09-06）→ 段数で傾きを急にした（下限をさらに下げるとこもる）。
  あわせて弱い音は**立ち上がりを鈍らせる**（`velocityToAttackSeconds`: pp で約 +0.02s、v=0 で +0.035s。0.5 以上は変えない）
