...
| パート間隔（段内の譜表間） | `staveSpacingForPartCount(partCount)` | `partSpacingOffsetPx`（全体設定、-20〜80px、Issue #90。上限は 2026-08-23 に 50→80 へ拡大＝月光級の深い音型+#382 クランプの逃げ場。既定値は Issue #199 から楽譜種別ごと＝ピアノ+38px・それ以外0） | localStorageキー（設定全体で1つ。段ごと・パート境界ごとの個別上書きは今回スコープ外） |
...
4. 値は #559 時点で 0.64、**2026-09-04 に運用者判断で 0.3**（dev の調整パネル #596 で月光検聴版・音符 150% を
   見ながら 0.4 → 0.3 と詰めて確定。月光 1〜9 小節の段割りは 2小節/段 → 3小節/段）。
   注意: 段割りの計画では下限ガード（項目2）が効くが、最終の小節幅は `allocateCombinedMeasureWidths` で
   均等幅とブレンドされるため、密疎が混在する段では密な小節が計画上の下限より縮み得る（#602/#603 と同じ構造）。
   0.3 での月光以外の譜種・重なりの実測は未記録（QA 資料の画像・数値は 0.64 時点）。
   Issue 本文の例（0.7〜0.75）より強い圧縮になった理由と、圧縮率ごとの段割りの実測は
...
（理想幅と固定ぶんを分けた9小節ぶん）を入力に「圧縮なしなら1小節/段・**0.64 なら2小節/段**（#559 時点の歴史的確認）・
**現行の既定 0.3 なら3小節/段**（2026-09-04 の運用者判断）」を固定する。0.3 での符頭の重なりの実ブラウザ確認は未記録。
jsdom では VexFlow の文字幅が 0 になり実ブラウザの半分以下の幅しか出ないため、症状そのものは
jsdom で再現できない。ブラウザでの実測（段割り・符頭の重なり・警告の消滅）は
`docs/qa/system-break-min-width/README.md` に前後の画像つきで残した（**0.64 時点**）。
