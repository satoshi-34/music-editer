...
### Undo を積まないという判断（#523 との違い）

Issue #572 の仕様2は「#539（=#523）に完全準拠」として Undo も挙げているが、**パート間隔だけは
履歴を積まない**ことにした。理由:

- パート間隔は譜面データではなく **ブラウザに保存する表示設定**（`localStorage` の
  `score-part-spacing-offset`）で、Undo/Redo のスナップショット（`ScoreSnapshot`）に入っていない
- レイアウトタブのスライダーも履歴を積んでいない（動かしても「元に戻す」の対象にならない）
- ここでドラッグにだけ `pushHistory()` を積むと、「元に戻す」が**パート間隔を戻さずに譜面の中身を
  1手戻す**という食い違いになる（履歴に積まれるのは音符などのスナップショットであり、
  パート間隔は含まれないため）

したがって共通部品の `onDragStart` / `onDragEnd` には何もしない関数を渡している。
「ドラッグが履歴を1件も消費しない」ことは `ScorePagePartGapDrag.test.tsx` で固定した。
パート間隔まで Undo の対象にしたい場合は、まず設定値を `ScoreSnapshot` へ入れる（＝スライダーも
含めた設計変更）必要があり、本Issueの範囲を超える。
...
- **段ごとのパート間隔の個別化**（#449）。今回動くのは全体設定なので、どの段で掴んでも譜面全体の
  パート間隔が変わる
- **#571（レイアウトタブの「整えるモード」＝調整ハンドルの常時表示）への組み込み**。#571 は
  PR #614 でレビュー中のため、この帯は従来どおり「段を選択している間だけ」出る。#571 が main へ
  入った時点で、常時表示の対象へこの帯も含める（Issue #572 の仕様4はその時点で満たす）
- **五線の中の当たり判定との調整**。帯は段を選択している間だけ五線の間へ重なるので、その間は
  帯の下の音符をクリックできない（選択を解けば元どおり）。#571 の常時表示へ組み込むときは、
  この重なりを再検討する必要がある
...
- 追加テスト: `src/components/ScorePagePartGapDrag.test.tsx`、`AppCssSystemSelectPrint.test.ts` に1件
