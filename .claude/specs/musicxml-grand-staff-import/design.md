...
- **連符グループ境界の是正（2026-08-29 続報）**: 「不完全グループ38件は混合音価」という
  ...
  （109グループ全て完全一致）。単体テスト5件+ScorePage 実マウント1件（明示マーカー分離・個数フォールバック・明示混合音価の id 保持・マーカー無し混合の結合維持・並び順の誤分割防止・三連×3の vf-tuplet 描画）で固定。混合音価グループは正しく区切れても描画側が連符括りを出さない既知の制約が別にある（tuplet-implementation 設計書の既知の制約②）
