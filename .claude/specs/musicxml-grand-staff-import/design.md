...
1. **要素のふるい分け**: `direction` はその五線に属さないものを落とす。**別五線の `note` は
   ...
   独立扱いすると同時発音が分裂する）。連符（time-modification 付き）が五線をまたぐ形だけは
   合成休符が 1/3 拍等を表せないため、理由付きでエラーにする
...
- **クロススタッフ連符（P1）**: 合成休符（二進音価）で 1/3 拍を表せないため、理由と代替手順付きで
  ...
  **「同じ音価・付点・連符比を持つ休符」への1:1置換**で読み込む（時間は厳密に保存される）。
  グループ id は連続する置換対象ごとに採番（間に自五線の音や backup が挟まれば別グループ。
  見た目の括りが元と変わり得るが ⇵ で付け直せる）。actual/normal が壊れていて比を復元
  できない場合だけ従来どおり理由付きで中止（いずれも単体テストで固定・実物 97小節で検証済み）
...
- **既知の別課題**: 混合音価の連符（4分+8分の三連等、イベント数≠numNotes）は描画側が
  連符と認識しない既存制約で、本対応の範囲外（実物ソナチネでは 48 グループ中 38 が該当）
