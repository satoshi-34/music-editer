...
2. プルダウンで最後に選んだ連符（`useState`。**作品データには保存しない**＝セッション内保持）
3. 既定の3連符（`DEFAULT_TUPLET_NUM_NOTES`）

2 を state に持つのは、ONにした連符を外した直後（別の音価ボタンを押した等）でも
ボタンに選択が残り、次が1クリックで済むようにするため。作品をまたいで持ち越さないのは
仕様3（作品データには保存しない）どおりで、リロードすると3連符へ戻る。
...
- `src/components/ToolVariantButton.tsx`: #548 のブランチから**そのままコピー**（差分なし）。
  先にマージされた側の1枚だけが残る
...
- **連符数字の表示切替（#269）・段またぎ表示（⇵）は畳んでいない**。
  ...
  連符ボタンの集約だけなので広げない
- **キーボードショートカットは追加していない**。プルダウンは ▾ ボタン（`aria-haspopup="menu"`・
  `aria-expanded`）とメニュー内のボタン列（`role="group"`）で構成され、Tab と Enter で
  操作でき、Escape で閉じる（`ToolVariantButton` の実装）。数字キーで連符を選ぶ等の
