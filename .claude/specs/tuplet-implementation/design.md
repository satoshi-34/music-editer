...
- ~~MusicXML インポート側は `<tuplet type="start/stop">` を見ていない~~ →
  **2026-08-29 に解消**（隣接する同一比率の連符が誤結合され連符表示が消える実害が
  ソナチネ実ファイルで発生）。現在は `<tuplet type="start/stop">` を最優先の境界とし、
  マーカーの無い出力は「均一音価を numNotes 個で切る」先読みフォールバックで分割する。
  マーカー無し+混合音価の連続グループのみ境界を判定できず結合のまま読む（既知の制約①）。
  さらに**混合音価グループ（4分+8分の三連等）は、明示マーカーで正しく区切れても描画側が
  連符と認識しない**（createVexFlowTuplets の「イベント数=numNotes」条件。既知の制約②・
  読込データの id・時間は正しく保持されるが、括りと VexFlow 側の tick 倍率が出ない。
  描画対応は別課題）
...
（出典: ユーザーフィードバック 2026-08-29「N連符ボタンで二連符とかも選べるように」）。
...
- **MusicXML 読込は未対応**（`show-number` を読んで `hideNumber` を復元しない）。
  ※グループ境界の判定は 2026-08-29 から `<notations><tuplet start/stop>` を最優先に変更済み
  （上の「既知の制約」の項を参照）だが、`show-number` の読込は引き続き未対応。
  トリアージの範囲（書出）に絞った経緯は当時のまま。**往復すると非表示指定は失われる**
...
## 追記12: 連符数字が五線をまたいで反対側へ取り残される（Issue #471）

### 問題

ユーザーフィードバック（2026-08-29 受領・Debussy「String Quartet G-minor」Op.10）で
「三連符の記号がビームと重なる」と報告された。

原因は VexFlow の `Tuplet` の縦位置の決め方にある。

- 上下どちらに置くか（`location`）は **符幹（stem＝符の棒）の向きだけ**で決まる
  （`Beam.generateBeams` が `tuplet.notes[0].stemDirection` を見て `setTupletLocation` する）
- 縦位置（`Tuplet.getYPosition`）は「第1線の 1.5 間ぶん上」「第5線の 2 間ぶん下」を**起点**にし、
  そこから**外側へしか動かない**（`Math.min` / `Math.max` で更新するだけ）

この2つが噛み合うと、**符幹が五線の内側を向く配置**――加線の上に離れた高い音符に
下向き符幹が付く場合など――で、数字だけが音符から五線をまたいで反対側へ取り残される。
単一の五線なら「離れて見づらい」で済むが、**多段譜では下の段の五線・ビームの上に重なる**。
弦楽四重奏のように段の間隔が狭い譜面で顕在化する。

### 実測（修正前・実ブラウザ / 大譜表・`scale=1`）

第1線 y=60.5 / 第5線 y=100.5、下の段の第1線 y=140.5 の譜面で、
上の段に `c/6 d/6 e/6` の8分3連（符幹下向き・ビームは y=75〜80 で**五線の中**）を置くと:

| | 修正前 | 修正後 |
|---|---|---|
| 連符数字のベースライン y | **129.66** | **15.66** |
| 自分のビーム（y=75〜80）との距離 | 約5間ぶん下 | 音符の上（隣接） |
| 下の段（第1線 y=140.5）との関係 | 段間へ張り出し、隣接する下段のビームと重なる | 干渉しない |

同じ譜面の他の連符（音符が五線にかかっているもの）は
`y=38.659` / `y=144.66` のまま**修正前後で不変**であることも実測で確認した。

### 修正設計

`src/utils/vexFlowTimingUtils.ts` に `syncTupletPlacementWithNotes` を追加し、
**描画の直前**（音符が五線へ紐づき、ビームが確定したあと）に置き場所を見直す。

- 連符の**符頭がすべて第1線より上**にあるなら `Tuplet.LOCATION_TOP` へ
- 連符の**符頭がすべて第5線より下**にあるなら `Tuplet.LOCATION_BOTTOM` へ
- それ以外（音符が五線にかかっている＝大多数）は VexFlow の判断のまま

「音符が五線の外に出ているときだけ、音符と同じ側へ寄せる」という限定なので、
既存譜面の見た目はほとんど動かない（上の実測のとおり）。

呼ぶ場所が `syncTupletBracketsWithBeams`（連符生成の直後）ではなく描画段なのは、
`note.getStave()` が必要だから。生成段では音符がまだ五線に紐づいておらず、
位置関係を判断できない（紐づいていない場合は何もせず戻る）。

### 影響範囲

- `src/utils/vexFlowTimingUtils.ts` — `syncTupletPlacementWithNotes`（新規）
- `src/components/PianoSystemCanvas.tsx` — 連符の描画ループ直前で呼ぶ
- テスト: `src/utils/vexFlowTimingUtils.test.ts`（+4）/
  `src/components/PianoSystemCanvasTupletPlacement.test.tsx`（新規・3件）
  - 負のテストで検出力を確認済み: 呼び出しをコメントアウトすると、
    高い音符側が `expected 122 to be less than 60.5`、
    低い音符側が `expected 43 to be greater than 100.5` で落ちる

### やらなかったこと

- **段の間隔そのものの調整**は入れていない。音符が五線の中にあり符幹が外を向く連符
  （上の実測の `y=144.66` のケース）では、数字が下の段の領域へ入ることがあるが、
  これは連符の置き場所ではなく段間の縦配分の問題で、レイアウトの正本
  （`.claude/specs/layout-pipeline/design.md`）側の判断が要るため別件とした
- **完全な再現データでの照合**は未実施。Issue #471 本文のとおり状態JSONは途中までしか
  受領できておらず、トリアージコメントの指示（最小構成を自前で組んで再現する）に従った

## 追記13: MusicXML 書き出しの連符 duration が丸め誤差で小節合計を崩す（Issue #519, 2026-09-01）

### 問題

MusicXML 書き出しの `<divisions>`（4分音符1つを何単位で表すか）が **16 固定**だったため、
連符の duration が整数にならず四捨五入されていた。

- 8分3連1つ = `16 ×(1/2)×(2/3) = 5.33…` → **5 に丸められる**
- 3連×12個で埋めた 4/4 の小節は合計 `5×12 = 60`。本来の `64`（= 16×4拍）に **4単位不足**する

運用者QA（2026-08-31）で、アプリで打ち込んだ月光（8分3連の連続・26小節・
time-modification 108個）の書き出しファイルで実際に確認された。往復すると拍構造が
崩れるため、読み込み後の段割りにも影響していた。

なお「読み込むとほぼ全段が1小節/段へ膨張する」現象そのものの真因は**読み込み側**であり、
Issue #526 に切り分け済み（本件のトリアージコメントで受入条件2が #526 へ移管された）。
本追記の範囲は**書き出しの correctness（拍合計の整合）**に限る。

### 採用した設計

`<divisions>` を固定値ではなく、**その譜面で使われている連符すべての duration が
整数になる最小の値**として書き出し時に決める（`resolveDivisions`）。

1. 音符1つが要求する倍率を求める（`requiredDivisionsScale`）。
   duration は「音価 × 付点倍率 × (notesOccupied / numNotes)」なので、
   分母 `numNotes` のうち分子と約分しきれない分だけ基準値を引き上げればよい。
   付点は 3/2・7/4 の**分数**として扱い、浮動小数の誤差を判定に持ち込まない
2. 譜面全体（全パート・全小節・**全声部**）でその最小公倍数を取り、
...
| 連符 | 音価 | 比 | 必要な倍率 | divisions |
| --- | --- | --- | --- | --- |
| 3連符 | 8分 | 3:2 | 3 | 48 |
| 5連符 | 8分 | 5:4 | 5 | 80 |
| 6連符 | 16分 | 6:4 | 3（16と6の公約数2で約分される） | 48 |
| 7連符 | 16分 | 7:4 | 7 | 112 |

**連符が1つも無い譜面では倍率が1のまま**なので、`<divisions>16</divisions>` と
各 duration は従来と1バイトも変わらない（回帰を出さないための設計）。
...
- `src/utils/musicXmlExport.ts`
  ...
  - `noteToXml` / `measureToXml` に `divisions` を引き回す
- テスト: `src/utils/musicXmlTupletDivisions.test.ts`（新規・9件）/
  `src/utils/musicXmlTuplet.test.ts`（既存の `<duration>5</duration>` 期待を更新）

実データでの確認: `docs/qa/regression/moonlight-bars1-9.score.json` を書き出すと
`divisions=48` が選ばれ、全27（パート×小節×声部）のうち26が `48×4=192` ちょうどに一致した。
残る1つ（P1 小節9 声部1）は fixture 側が4分音符1つしか持たない**不完全な最終小節**で、
データどおりの出力である（丸めによる不足ではない）。

### やらなかったこと

- **連符でない複付点64分音符などの丸めは従来どおり残す**。
  `1 × 1.75 = 1.75 → 2` のような丸めは divisions=16 の時点から存在するが、
  ここを直すと連符の無い譜面の出力バイトが変わり受入条件3（回帰なし）に反するため、
  倍率の計算対象は連符を持つ音符だけに限定した
- **読み込み側の許容**（既に出回っている 5.33→5 のファイルをどう読むか）は変更していない。
  Issue #519 のトリアージどおり、新規書き出しの correctness を先行させた（#526 の範囲）
- **MIDI 書き出し**（`src/utils/midiExport.ts`）は別の分解能を持つため触っていない

---

## 連符ボタンの集約: 6個 → 1個 + ▾（Issue #569）

### 問題

音符・休符タブに 2/3/4/5/6/7連符のボタンが6個横並びで常設されていた。
使用頻度は3連符が圧倒的で他は稀なのに、このタブだけで常時 49 個という過密
（`.claude/specs/toolbar-organization/design.md` §4）の一角を占めていた。

### 修正設計

**「代表1個 + ▾ で変種を選ぶ」形**に畳む。運用者指示は「連符ボタンはデフォルト3連符で、
プルダウンみたいなのを押すとn連符にできる感じで」。

| | 変更前 | 変更後 |
| --- | --- | --- |
| 列に並ぶボタン | 6個（2/3/4/5/6/7連符） | 2個（`3連符` + `▾`。実測幅 30px + 15px） |
| 3連符を使う | 1クリック | 1クリック（**変えない**） |
| 5連符を使う | 1クリック | 2クリック（▾ → 5連符。選んだ時点で有効） |
| 選んだ連符 | ボタンは常に固定 | ボタンの表示が選んだ連符に変わり、セッション内は残る |

**部品は #548（臨時記号の ♯▾/♭▾/♮ 統合）と共用**する。同じ形のプルダウンを
2枚作らないため、先行して部品を作った側（#548）の `src/components/ToolVariantButton.tsx`
をそのまま使う（「先行した方が部品を作る」という運用者方針）。

**ボタンに出す連符の決め方**（`Palette.tsx`）。臨時記号側と同じ順序にそろえてある:

1. いまONになっている連符（ツールが外から変わっても表示と実態がずれない）
2. プルダウンで最後に選んだ連符（`tupletVariantKey`。**作品データには保存しない**）
3. 既定の3連符（`DEFAULT_TUPLET_NUM_NOTES`）

2 を覚えておくのは、ONにした連符を外した直後（別の音価ボタンを押した等）でも
ボタンに選択が残り、次が1クリックで済むようにするため。

**選択の寿命（1文）: アプリを開いているあいだ持ち越し、リロード（再読み込み）で既定の3連符へ戻る。**
これを満たすため、選択は `Palette` ではなく **`ScorePage` の `useState`** に置く（round1 P2）。
`Palette` は音符・休符タブ以外へ移るとアンマウントされるので、そこに置くと
「5連符 → 演奏記号タブ → 戻る」で3連符へ戻ってしまう。`ScorePage` は
アプリ起動から終了までマウントされたまま（`App.tsx` はホーム表示中も譜面画面を
アンマウントしない）なので、タブ切替でも作品切替でも選択が残り、寿命が
「リロードまで」の一点に定まる。`Palette` 側は `tupletVariantKey` /
`onTupletVariantKeyChange` を受け取るだけの制御コンポーネントにしてある。

### 影響範囲

- `src/components/Palette.tsx`: `TUPLET_KINDS.map(...)` の6ボタンを `ToolVariantButton` 1つへ置換。
  ラベル生成を `tupletVariantLabel()` へ切り出し（本体ボタンとメニュー項目で同じ文言を使うため）
- `src/components/ToolVariantButton.tsx`: 共通部品。round1 の差し戻し（下記）を受けて
  **このブランチ側で作り直した1枚を正本**とし、#612（#548）はマージ後にこのファイルを
  取り込む（先にマージされた側の1枚だけが残る）
- `src/components/ScorePage.tsx`: 選んだ連符（`tupletVariantKey`）の保持と、
  音符タブの `Palette` への受け渡し
- `src/utils/toolbarPlacement.ts`: `clampDropdownMenuLeft()` と
  `estimateVariantMenuWidth()` を追加（既存の `clampDropdownMenuTop()` と対で使う）
- ラベルは従来と同じ `N連符（N:M）: …`（先頭一致で探している既存テスト・
  `BackwardCompatibility` の `3連符` を壊さない）
- 文字サイズだけ 11px → 10px。▾ のぶん本体ボタンが 36px → 28px に細くなるため
  （実測 `scrollWidth === clientWidth === 28` で、はみ出していないことを確認済み）

### 受入テスト

| ケース | テスト |
| --- | --- |
| 列が1ボタン+▾になる（2〜7連符は閉じている間は出ない） | `PaletteTupletVariant.test.tsx` |
| 3連符は1クリックON / もう一度でOFF | 同上 |
| ▾ で選ぶと即有効・表示も変わる・メニューは閉じる | 同上 |
| OFFにしても選択が残り、次は1クリック | 同上 |
| 既定3連符が譜面まで届く（保存データ 3:2・`vf-tuplet` 描画） | `ScorePageTupletVariantWiring.test.tsx` |
| ▾ →5連符 の2クリックが譜面まで届く（保存データ 5:4） | 同上 |
| 2連符・4連符（#472）が ▾ 経由でも従来どおり置ける | `ScorePageDupletWiring.test.tsx`（操作をプルダウン経由へ更新） |

### やらなかったこと

- **連符数字の表示切替（#269）・段またぎ表示（⇵）は畳んでいない**。
  toolbar-organization §7(d) では「表示 ▾」へまとめる案があるが、本Issueの仕様は
  連符ボタンの集約だけなので広げない
- **キーボードショートカットは追加していない**。プルダウンは ▾ ボタン（`aria-expanded`）と
  メニュー内のボタン列（`role="group"`）で構成され、Tab と Enter で操作でき、
  Escape で閉じる（`ToolVariantButton` の実装）。数字キーで連符を選ぶ等の
  割り当ては #563 のラベル規約の範囲外なので入れていない
- **矢印キーでのメニュー内移動は入れていない**。`role="menu"` を名乗るなら矢印キーまで
  必要になるが、ここは「並んだボタンの塊（group）」として一貫させる方を選んだ（下記 round1）

---

## round1 の差し戻しへの対応（#569・共通部品の作り直し）

レビューの差し戻し理由は3点とも共通部品 `ToolVariantButton.tsx` の設計不備だった。
**同じ部品を #612（臨時記号 #548）も持っているため、修正はこの #613 側の1本に寄せ、
#612 はマージ後にこのファイルを取り込む**（部品の2枚目を作らない・運用者方針）。

### P1: プルダウンがツールバーのパネルに切られる

`position: absolute; top: 100%` で出していたため、親 `.toolbar-panel`
（`overflow-x: auto`＝`src/App.css:175`。左＝縦配置ではさらに `overflow-x: hidden`＝`:222`）
にメニューが切られ、項目の大半が見えなかった。

**既存のリセットメニュー（`App.css:448-452` のコメントに同じ理由が書いてある）と同じ方式へそろえた**:

- `position: fixed` + ▾ ボタンの `getBoundingClientRect()` 実測位置
- 画面内へのクランプは既存ヘルパーを共用する。縦は `clampDropdownMenuTop()`（`menuMaxHeightPx`
  に自分の高さを渡す。既定の 420px はリセットメニュー用の大きな値で、この1行メニューに
  使うと画面下部で不必要に上へ跳ねるため）、横は新設した `clampDropdownMenuLeft()`
- 幅・高さは開いた直後は実測できない（描画前）ので `estimateVariantMenuWidth()` で
  見積もり、描画後に `useLayoutEffect` で実測に置き直す
- 開いているあいだは `resize` と `scroll`（capture。スクロールするのは window ではなく
  `.toolbar-panel` 側）で位置を測り直す。fixed は画面基準なので、追従させないと
  メニューだけが取り残される

配線テスト（実クリック）はクリップされた要素にもクリックが届くのでこの不備を検出できない。
`ToolVariantButtonPlacement.test.tsx` で **ボタンの矩形を偽装して座標そのものを検証**する。

### P2: 選択がタブ切替で消える

上記「選択の寿命」のとおり `ScorePage` へ持ち上げた。

### P2: ARIA の名乗りとフォーカス

- `aria-haspopup="menu"` を**外した**。中身は `role="group"` + 通常のボタン列なので、
  名乗り（menu）と実体が食い違っていた。矢印キーまで実装して `menu`/`menuitemradio` へ
  そろえる道もあるが、項目6個の1行メニューに矢印キー操作を足す利得が薄いので
  「グループで一貫させる」側を選んだ
- 項目選択・Escape の後に **▾ ボタンへフォーカスを戻す**（`closeAndRefocus`）。
  戻さないとフォーカスが文書先頭へ落ち、キーボード利用者が現在地を見失う

### 追加した受入テスト

| ケース | テスト |
| --- | --- |
| メニューが `fixed` で、ボタンの実測位置の直下に出る | `ToolVariantButtonPlacement.test.tsx` |
| 画面右端のボタンから開いても全項目が画面内（横クランプ） | 同上 |
| 縦配置で画面下部のボタンから開いても画面外へ出ない（縦クランプ） | 同上 |
| `aria-haspopup` を名乗らない／`aria-expanded` が開閉に追従 | 同上 |
