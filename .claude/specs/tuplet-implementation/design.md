...
9連符（9:8）は MusicXML 書出の分割数 `DIVISIONS = 16` では 8/9 倍が整数にならず
（8分音符なら 8 × 8/9 = 7.11… → 7 へ丸め）、往復すると小節の合計拍がずれる。
Issue #519 で扱っている丸め問題と同じ原因なので、そちらの解決前に選択肢を
増やすのは壊れた譜面を作りやすくするだけと判断した。2連符（3/2倍）・
