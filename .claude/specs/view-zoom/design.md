...
# ズーム上限の拡大 150% → 300%（Issue #176, 2026-08-04）

## 背景・問題
- 吹奏楽プリセット（10パート）などの総譜は、A4幅の1段（システム）に全パートを積む。
  そのため1五線あたりの表示が物理的に小さくなり、「画面表示のズーム」を上限の
  150% にしてもパート名・音符が細かく、クリック操作がしづらいという実機
  フィードバックが出た（運用者確認・2026-08-03）。
- 「音符の大きさ」（`notationSizeMultiplier`、80〜200%）を上げれば見た目は大きく
  なるが、こちらは `SCORE_LAYOUT_RENDER_SCALE` に掛かる倍率で**印刷結果と段組み
  （1段に入る小節数・ページ数）まで変わる**。画面で細部を確認したいだけの用途に
  使うのは筋が悪く、確認のたびに元へ戻す運用も現実的でない。
- 一方「画面表示のズーム」は `--scale`（`transform: scale()`）を掛けるだけの
  画面専用の倍率で、印刷（`@media print` で `transform: none !important`）にも
  譜面データにも影響しない。用途に合っているのはこちらなので、上限だけを広げた。

## 修正設計
- **上限値の定数化**: `src/utils/viewZoomUtils.ts` に `VIEW_ZOOM_MAX = 3` を追加し、
  `VIEW_ZOOM_MIN`（0.5）と同じ扱いにした。`ScorePage.tsx` 側に散っていた
  マジックナンバー `1.5`（state 初期化時のクランプ・スライダー `onChange` の
  クランプ）と、スライダーの `min`/`max` 属性の両方をこの定数から導出する形に
  そろえ、「片方だけ直して範囲がズレる」事故を起こせないようにした。
  刻み（5%）と下限（50%）は変更していない。
- **`computeFitZoom` は変えない**: 初期ズームの幅フィット（issue #40）は
  `Math.min(1, ...)` で 100% 頭打ちのまま。ここは「初期値をどう決めるか」の計算で、
  スライダーの可動域とは意味が異なる（起動時にいきなり300%で開くのは不適切）。
  紛らわしいので、その旨をコード上のコメントとユニットテストで固定した。
- **合成方式・座標系・印刷は一切変更なし**: `effectiveScale = scale * viewZoom` の
  合成、`--scale` を経由するヒットテスト（`clientToGroup` / `getRawPerScreenPx` が
  `svg.getBoundingClientRect()` の実測値からスケールを求める方式）、`@media print`
  での解除は、いずれも倍率の値に依存しない作りのため素通しで追従する。
  つまり本変更は「スライダーが取りうる値の範囲」だけを広げたものである。
- **localStorage 互換**: 保存形式（キー `score-view-zoom`、内部倍率の文字列）は
  従来どおり。150% を超える値も同じ形式で保存・復元される。逆に、壊れた値・
  範囲外の値は新しい上限 3.0 へクランプされる（従来と同じ防御をそのまま流用）。

## 影響範囲
- `src/utils/viewZoomUtils.ts`: `VIEW_ZOOM_MAX` を追加（`computeFitZoom` の
  ロジックは変更なし。100%頭打ちの理由をコメントで明記）。
- `src/components/ScorePage.tsx`: `VIEW_ZOOM_MAX` を import し、`viewZoom` state の
  クランプ・スライダーの `min`/`max`・`onChange` のクランプを定数参照へ変更。
  ツールチップ文言に「最大300%まで上げられます」を追記。
- `src/utils/viewZoomUtils.test.ts`: 上限値と「フィット計算は上限拡大の影響を
  受けない」ことのテストを追加。
- `src/components/ScorePageViewZoomRange.test.tsx`（新規）: スライダーの
  min/max/step、300% 時に実際の `--scale` が 150% 時の2倍になること、
  150% 超の保存値の復元、範囲外保存値のクランプ、既定 100% の維持。
- `src/components/PianoSystemCanvasZoomSelect.test.tsx`: 既存の 150% 固定の
  検証を `describe.each([1.5, 3])` でパラメータ化し、新しい上限でも
  座標変換（個別音選択の許容幅・クリック位置→音高）が正しいことを確認する
  テストを追加。
- `README.md` / `docs/DEVELOPMENT.md`: 可動域の記載と最小チェックを更新。

## 検証
（PR本文と同じ内容を要約）
- `npx vitest --run src` / `npm run lint:ratchet` / `npm run build` をworktree内で実行。
- 追加テストは変更前のコードに対して落ちる（＝素通りしない）ことを確認済み。
- ブラウザ確認は吹奏楽プリセット（10パート編成譜）で 150% → 300% と上げ、
  表示サイズが2倍になること・クリック入力/音符選択が狙った位置に効くこと・
  リロード後も300%が保持されること・印刷プレビューが原寸のままであることを確認。

---

# 追補: 「表示領域の幅」の測り先を `.paper-rail` から `body` へ変更（Issue #212, 2026-08-09）

上の2節（#40 の初期ズームの幅フィット・`useAutoPageScale` の自動縮尺）は、どちらも
**`.paper-rail` の `clientWidth`** を「表示領域の幅」として読んでいた。

Issue #212（横スクロール時に右側の背景が白く抜ける）の修正で `.paper-rail` に
`min-width: min-content` を入れたため、**拡大表示ではレール自身が中身の幅まで広がる**ようになり、
レールを測ると「広がった幅」＝自分が出した結果を測り直す循環参照になってしまう。

そこで `src/utils/viewZoomUtils.ts` に `readPageAreaAvailableWidth(el)` を追加し、
両方の測定を `body` の `clientWidth` へ寄せた。`body` は中身がはみ出しても広がらないので
「ウィンドウの幅」の代表として安定しており、**はみ出しが無いときの値は従来のレール幅と一致する**
（`.app-root` / `#root` に左右の余白が無いため。実測でどちらも 1265px）。
自動縮尺の計算式（`need = pageWidthPx * columns + gap`）・ヒステリシス・ズームの合成方法は変更していない。

詳細な背景と実測値は `.claude/specs/bug-fixes/design.md` の
「ズームで横スクロールになると右側の背景が白く抜ける問題（Issue #212）」を参照。
