...
- 読み込み（`src/utils/musicXmlImport.ts`）: C 記号のとき `line` が `4` ならテナー記号、
  ...
  ~~ただし、小節単位の途中クレフ変更の読み込みは対象外とした。
  パートごとの単一の既定クレフ（先頭の `<attributes><clef>`）のみ復元する。~~
  → **Issue #453（段2）で読み込みにも対応**（このファイル末尾の「段2」の節を参照）。
...
1. ~~**MusicXML 読み込みは小節単位の途中クレフ変更に対応しない**。パートごとの先頭の
   ...
   （途中クレフ変更）は無視する。~~
   → **Issue #453（段2）で解消**。小節頭・小節途中とも読み込むようになった
   （このファイル末尾の「段2」の節を参照）。
...
- **パディング休符のキーとクレフの食い違い（P2）**: round1 対応は休符キーだけを
  開始拍のクレフで生成し、makeVFNote へ渡すクレフ（clefForVoiceEvent）は最後の実イベントへ
  フォールバックしたままだった。補完休符ごとのクレフ台帳（paddingRestClefs）を作り、
  キー生成と clefForVoiceEvent の両方が同じ台帳を参照するように統一。
  回帰テストは「途中変更あり/なし/全ヘ音基準」の3描画比較で、変更後の拍の補完休符が
  **全ヘ音描画と同じ高さ**になることまで固定（「位置が動いた」だけの判定では、誤った位置へ
  動くケースを見逃すことを負のテストで実証済み。台帳参照を外すとこのテストは失敗する）

---

# 段2: MusicXML 読み込みでの途中クレフ変更の取り込み（Issue #453）

## 問題

段1（#424）で入力・描画・**書き出し**は揃ったが、読み込みは小節途中の
`<attributes><clef>` を一切見ておらず、パート先頭の `<clef>` だけを既定クレフとして
読んでいた（上の「現状の制限」1）。そのため

- 自前で書き出した MusicXML を読み直すと、途中クレフ変更（`MeasureData.clef` /
  `NoteEvent.clefChange`）が黙って消える（往復で情報が落ちる）
- Finale からの持ち込み（#419 経路）でも、曲の途中でクレフが変わる譜面が
  「最初のクレフのまま」読まれる（月光ソナタの往復に必要）

## 修正設計（`src/utils/musicXmlImport.ts`）

### 1. どこで変わったかは「並び順」ではなく「時間」で決める

`collectMeasureClefs(measureEl, staffNumber, divisions)` を新設した。小節の子要素を
文書順にたどりながら MusicXML の時間カーソル（音符と `<forward>` で進み、`<backup>` で
戻る。和音の構成音 `<chord/>` と前打音 `<grace/>` は進めない）を追い、`<attributes>` に
その五線のクレフがあれば**そのときのカーソル位置**とともに記録する。

- カーソルが 0 のもの = `headClef`（小節の頭のクレフ）
- カーソルが 0 より後のもの = `midClefs`（`{ beat, clef }`。beat は 4分音符を1とする拍）

並び順ではなく時間で見るのは、`<backup>` で区切られた下声の側にクレフ変更が書かれている
ファイルでも、同じ時刻の主声部の音へ正しく結び付けるため（クレフは声部ではなく**五線**の
性質なので、下声側に書かれていても五線全体が変わる）。

### 2. 五線の取り違えを防ぐ（`clefForStaffExact`）

既存の `clefForStaff` は「番号が一致する `<clef>` が無ければ先頭の `<clef>` を使う」
フォールバックを持つ（パート既定クレフの推定にはこれが要る）。小節途中のクレフに
同じフォールバックを使うと、`<clef number="1">`（上段だけの変更）を下段の読み取りでも
拾ってしまい、**指示されていない段のクレフまで変わる**。そこで小節内のクレフ収集専用に
`clefForStaffExact`（該当が無ければ `null`）を用意した。MusicXML の `number` の既定値は
1 なので、番号なしの `<clef>` は第1五線を指す扱いにしている。

### 3. データ形への戻し方

`buildStaffMeasures` はパート全体を通して次の2つを持ち回る。

- `runningClef`: 前の小節の末尾時点で有効なクレフ。「変更なのか、同じクレフの念押し表記
  なのか」を判定する比較相手
- `carriedClef`: 小節の末尾（最後の音より後ろ）に置かれた予告クレフ

小節ごとの処理は次のとおり。

- **小節頭**（`headClef`、または前の小節から持ち越した `carriedClef`）が `runningClef` と
  違うときだけ `MeasureData.clef` にする。毎小節クレフを書き直す書き出し元でも、
  変わっていなければ何も足さない（保存内容を増やさない・小型クレフを増やさない）
- **小節途中**（`midClefs`）は、主声部イベントの開始拍を積算した表と突き合わせ、
  その時刻以降に始まる最初のイベントの `NoteEvent.clefChange` にする。
  `clefChange` は「このイベントの直前に小型クレフを置き、このイベントから有効」の意味
  （段1の定義）なので、音が伸びている途中に書かれたクレフは次に始まる音から有効になる
- 最後の音より後ろにあるクレフは `carriedClef` へ回し、**次の小節の頭**からの変更として
  読む（実譜の予告クレフと同じ扱い）

`clefChange` を主声部だけに付けるのは段1の v1 方針のまま（追加声部にも持たせると
同じ時刻に声部ごとの別クレフを主張できてしまう）。

## 影響範囲

- `src/utils/musicXmlImport.ts`: `clefForStaffExact` / `collectMeasureClefs` 新設、
  `buildStaffMeasures` に `runningClef` / `carriedClef` の持ち回りと
  `MeasureData.clef` / `NoteEvent.clefChange` の書き込みを追加
- 読み込み経路（ScorePage の MusicXML / .mxl 読込、#419 の大譜表分割）はそのまま使える。
  解決・描画・書き出しは段1の実装をそのまま通る

## テスト

- `src/utils/musicXmlClef.test.ts`（読み込みの describe を追加）
  - 小節途中の `<attributes><clef>` が、その位置の音符の `clefChange` になる
    （他の音にはプロパティが生えない）
  - 小節頭の `<clef>` は前の小節から変わったときだけ `MeasureData.clef` になる
    （同じクレフの書き直しは取り込まない）
  - 小節末尾の予告クレフは次の小節の頭からの変更として読む
  - 大譜表（`<staves>2`）で `<clef number="1">` は上段だけに効く（下段は巻き添えにしない）
  - export → import の往復で `clefChange` と小節単位の `clef` が保たれる
- `src/components/ScorePageMusicXmlGrandStaff.test.tsx`（実マウントの読込統合テスト）
  - 小節途中に `<clef>` のある大譜表をファイル選択から読み込むと、自動保存データに
    `clefChange` が入り、小型クレフ（20pt）が SVG に描かれる

## 残っている制限

- 読み込み時の**既定の休符表示位置**（`defaultRestDisplayKeyForDuration`）は、従来どおり
  パートの既定クレフで決める。クレフが変わった後の小節の休符は、表示位置が本来より
  ずれることがある（音高データには影響しない表示専用の値）
- ⇵（段またぎ）との共存、パート譜表示の追随確認は段1から引き続き対象外

## 段2 Codex round 1 対応（2026-08-29）

- **[P1] 文書順による適用順の誤り**: `<backup>` で戻った下声側に書かれたクレフは、
  文書順では拍位置の前後が入れ替わり得る。midClefs を拍位置で安定ソートしてから
  適用するようにした（テスト: backup+forward で拍2のアルトを拍3のヘ音より後に書いた
  ファイルで、時間順（アルト→ヘ音）に適用され、次小節の同じヘ音の書き直しを
  取り込まないこと）
- **[P1] 予告位置での複数回変更**: 最後の音より後ろで treble→bass→treble と変わると、
  単純な runningClef 比較では最後の treble を念押しと誤判定して bass を持ち越していた。
  予告の比較相手を「現時点の末尾状態」（carriedClef ?? runningClef）にした
  （テスト: 正味の変更なしのときにどこにも指定が生えないこと）
- **[P2] ユーザー向け文書**: README の「読み込みではパートごとの既定クレフのみ復元」を
  現行動作（読み書き両対応）へ更新。設計書冒頭の旧制限の記述にも取り消し線と誘導を追加
