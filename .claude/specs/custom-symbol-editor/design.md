...
（`SymbolAdjustOverlay` の `useLayoutEffect` の依存は `anchor` だけ。`anchor` は
オーバーレイを開いた時点に作られたオブジェクトで、下書きの更新では差し替わらない）。
...
## 追補3（2026-08-24・Issue #389: 記号の「選択→Delete」統一）

**方針**: 「選択できる記号は Delete で消せる」を、弧・松葉だけでなく調整可能記号
（強弱・アーティキュレーション・カスタム記号・テキスト系）へ広げる。

### 論点1（オーバーレイを開くタイミング）の比較と結論

Issue の3案を、**削除動線が実際に成立するか**で比べた。

| 案 | 判定 | 理由 |
| --- | --- | --- |
| a. 選択と同時にオーバーレイを開く | **不成立** | ✥ オーバーレイは #205（矢印キー移動）のために開いた直後に入力欄へ autoFocus する。受入条件3「入力欄フォーカス中の Delete は文字編集を優先」を守る限り、**Delete が必ず文字編集へ吸われて記号が消せない**。「一度どこかをクリックしてフォーカスを外してから Delete」は動線として破綻している |
| b. 2段階（1クリック=選択・2クリック目=オーバーレイ） | **採用** | 1クリック→Delete で消せる（最短）。調整に入るには同じ記号をもう1回クリックする |
| c. 遅延（少し待つと開く） | 不採用 | a と同じ autoFocus の問題が時間差で起きるだけで、削除動線は成立しない。説明も難しい |

**採用範囲を限定した**（重要）: b を適用するのは「調整ツールを持っていないとき」だけ。
✥（symbolAdjustOffset）・⤢（symbolAdjustResize）を選んでいるときは、ツールボタン自体が
「これから調整する」というモード宣言なので、従来どおり**1クリックでオーバーレイを開く**。
PR #388 のルーティング（⤢→サイズ／同種ツール→トグル）はそのまま維持し、
「それ以外 → ✥ を開く」だった枝だけが「それ以外 → 選択（もう一度で ✥）」に変わる。

記号クリックの行き先（現在の全体像）:

1. ⤢ 選択中 → サイズ調整オーバーレイ（#388）
2. 同種の記号ツール選択中 → トグル（付いているものを外す・#388）
3. ✥ 選択中 → 位置調整オーバーレイ（従来どおり）
4. それ以外 → **1クリック目は選択のみ**（青枠＋通知）／同じ記号の2クリック目で位置調整

### 論点2（⤢ の動線）と統合パネル案の扱い

運用者コメント（2026-08-23）の**統合パネル案（記号クリック→位置＋サイズ＋削除を1枚に）は
この段では実装していない**。理由は次の3点で、いずれも「削除動線の統一」とは独立に
決着させる必要があるため（この段を待たせるより、先に削除動線を通したほうが弟テスト会に近い）:

1. ⤢/✥ ボタンを廃止すると、**音符クリック経由の選択リスト**（字面が小さい運指や重なった記号を
   音符側から確実に選ぶ動線・`symbolPicker`）の入口が無くなる。代替（音符の右クリック／長押し、
   またはパネル内に記号リストを持つ）の設計と実装が要る
2. ツールボタンは「モード宣言」でもあり、廃止すると #388 のトグル・ルーティングの意味づけを
   作り直す必要がある（トグル解除をパネル内の削除へ一本化するか、ツール一致時のトグルを残すか）
3. タッチでの誤タップ耐性（小さい字面への直接タップ精度）の検証が別途要る

この段の 4（選択のみ）は統合パネルへの**前提**になっている（クリック＝まず選択、という
意味づけが先に決まっていれば、2クリック目の中身を「位置＋サイズ＋削除の1枚」へ差し替えるだけで
統合パネルへ移行できる）。統合パネルの実装は別Issueで扱うことを提案する。

### 実装

| ファイル | 変更内容 |
| --- | --- |
| `src/utils/symbolAdjustUtils.ts` | `AdjustTarget` 型を PianoSystemCanvas から移設。`removeSymbolTargetFromEvent`（標準記号=`removeAdjustableSymbol` / カスタム記号=既存トグル）・`adjustTargetLabel`・`adjustTargetKey` を追加 |
| `src/components/PianoSystemCanvas.tsx` | 選択の union へ `symbol` スロットを追加（音符・弧・松葉と排他）。記号1件を消す `removeSymbolFromPartsScore` を module スコープに置き、**キーボードの Delete と ✥ オーバーレイの削除ボタンの両方がこの1本を通る**。当たり判定 rect に選択中クラス・青枠。声部切替・外部差し替え（Undo 等）での選択掃除、背景クリックでの解除も弧・松葉と同じ規則で追加 |
| `src/utils/scoreEditorNotices.ts` | `describeSymbolSelected`（選択時に「Delete で削除・もう一度クリックで位置調整」を伝える） |
| `src/App.css` | `.symbol-hit-region--selected` の青枠。画面用の `stroke: none !important` を打ち返しつつ、印刷・印刷プレビュー側の打ち消しルール（詳細度が高い）は効いたままにする |

**「同じロジックの2枚目」を作らないための措置**: 削除は `removeSymbolFromPartsScore` の1本、
対象の型と表示名・識別キーは utils の1組にそろえた（#223→#280 のような「片方に修正が届かない」
事故を避けるため）。記号の当たり判定を作っているのは PianoSystemCanvas だけ（StaffCanvas は
退役済み）なので、他コンポーネントへの写しは無い。

### 影響範囲・既知のリスク

- 保存データ・MusicXML の形は変えていない（選択は画面上の状態のみ）
- 挙動が変わるのは「調整ツールを持たない状態での記号クリック」だけ。既存テスト
  （`PianoSystemCanvasSymbolClickRouting` の ✥ 経路・`PianoSystemCanvasAdjustOverlayPlacement`）は
  2クリックへ更新した（設計時の期待を黙って書き換えないため、変更理由をテスト内コメントに残した）
- 削除の粒度は ✥/⤢ と同じ**種類単位**（dynamics は pp+cresc が両方消える）。#388 の削除ボタンと同じ
- Delete の受け口は `isTextInputTarget` 判定の**後**に置いた（弧・松葉は前）。記号の選択は
  調整オーバーレイ（数値入力欄）と同時に成立しうるため、入力欄の文字編集を必ず優先する

### 受入テスト（`PianoSystemCanvasSymbolSelectDelete.test.tsx`）

1. 記号クリックで選択（青枠クラス＋stroke）・オーバーレイは開かない・通知が出る
2. 選択中の Delete で記号と `symbolAdjust` が消え、通知が出て選択も外れる
3. Backspace でも消せる／アーティキュレーションでも同じ動線で消せる
4. 入力欄からの Delete では消えない（受入条件3）
5. Esc・譜面の空白クリックで選択が外れ、その後の Delete では消えない
6. 同じ記号の2クリック目で ✥ が開く
7. ⤢ 選択中は従来どおり1クリックでサイズ調整（選択を挟まない）
