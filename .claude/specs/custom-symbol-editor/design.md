...
**歌詞の配線漏れ（Codex round 2 P2）**: README/DEVELOPMENT は歌詞もクリック対象に
挙げていたが、`lyricsEntries` にパート・声部・イベントのメタデータが無く、
...
声部3は切替も小窓も出さず理由を通知／調整値が記号の属する声部へ書かれる／歌詞も
他レイヤーからクリックできる）。ScorePagePartSymbolsWiring.test.tsx に実経路の配線テスト
