...
大きく動かしたいときに何十回もキーを叩くことになっていた。ユーザーフィードバック（#450）の
...
0. **少数点の保護**（round1/round2 P2）: 実点数（閉曲線は閉じ用の重複終点を除く）が
   `MIN_POINTS_FOR_SMOOTHING = 8` 未満のストロークは無加工で返す。ジッターは密な
   サンプリングでしか生じず、数点の線は「クリックで置いた意図的な折れ線」
   （3点の山・疎な多角形）で、平均を掛けると意図した形そのものが壊れるため
1. **移動平均で震えをならす**（`SMOOTHING_PASSES = 2` 回）
   各点を前後の点との重み付き平均（0.25 / 0.5 / 0.25）で置き換える。1点ごとに上下する
   ジッターはこの平均でほぼ打ち消し合う。**開いた線では始点・終点は動かさない**
   （描き始め・描き終わりが縮むと線がつながらなく見えるため）。
   **閉曲線（始点＝終点）は円環として全点を循環近傍で均す**（round1 P2: 開いた線として
   端を固定すると図形が開始点側へ縮んで偏る。回帰は「開始位置を巡回シフトしても結果の
   形が一致する」テストで固定）
...
配線は ScorePage 実マウントの統合テスト（`ScorePageSymbolSmoothingWiring.test.tsx`）で固定:
エディタの補正切替 → 楽譜 svg の path d 変化 → オフで元の d へ完全復元 → 自動保存で
points 不変＋smoothing フラグのみ保存
