...
- パート譜固有のレイアウト（改行位置など） — **段A（パート単体の自動段割り）を実装済み**（Issue #174・2026-08-22。下記「第3段階 段A の実装記録」参照）。パート譜内での手動調整と保存（段B）は保存先の裁定待ち
...
- パート譜表示中に編集できるのは音符の入力・削除と記号・アーティキュレーション
  （第1・第2段階）。改行位置はパート単体で自動計算される（第3段階 段A）が、
  パート譜内での手動レイアウト調整とその保存（段B）は未対応（Issue #174 で裁定待ち）
...
## 第3段階 段A の実装記録（Issue #174・2026-08-22）

仕様は Issue #174 のコメント（2026-08-22 の調査結果と設計案）を正本とする。
段B（パート譜内の手動調整と保存）は保存先の裁定待ちのため、この段では実装しない。

### 問題

パート譜表示は1パートだけを描くのに、段割り（改行位置）の計画は総譜全パートの幅で
行っていた。段割り計画への入力 `layoutParts`（ScorePage）がパート譜表示を見ておらず、
常に全パートを含んでいたため。結果、抜き出したパートがスカスカでも総譜と同じ位置で
改行され、市販のパート譜のように詰めて印刷できなかった。

### 修正設計

ScorePage の3つの計算をパート譜表示対応にした（描画コンポーネント側は変更なし）:

1. **`layoutParts`**: パート譜表示中は選択パート1つだけを返す（編成譜の大譜表パートは
   従来どおり2段ぶん展開）。段割り計画（`planEffectiveMeasuresPerSystem` →
   `planSystemMeasureRanges`）は入力が絞られるだけで、計画ロジック自体は共通のまま
2. **`instrumentLabelAreaWidth`**: 描画側に合わせる。四重奏のパート譜
   （PartExtractionStaff）はパート名を五線左に描かない（showInstrumentLabels=false）ため
   余白0。編成譜のパート譜は1段目にだけ選択パートの名前を描くので選択パートのラベル幅。
   計画が描画より広い余白を見込むと、その分1段の小節数が不当に減る（計画≦描画なら安全）
3. **`plannedRanges`**: 総譜の「段ごとの小節数の上書き」（systemMeasureOverrides）を
   パート譜表示中は適用しない。総譜の紙面向けの調整であり、パート単体の自動計画と
...
- パート譜表示の画面・印刷・PDF（同じ plannedRanges を使うため自動で追従）
- 総譜表示は不変（受入3のテストで固定）。保存データも不変（段Aは描画時計算のみ）
- 段数/ページ（systemsPerPage）は総譜と共有のまま（1段が短いぶんページ内の余白は
  広めになる。パート譜専用の段数はレイアウト保存（段B）とまとめて扱う）

### テスト

`ScorePagePartSymbolsWiring.test.tsx` と同じ ScorePage 直接マウント方式の
`ScorePagePartLayout.test.tsx`。スカスカな Violin I・密集した Violin II・
1段目=1小節の総譜上書きを仕込んだ四重奏で、受入1〜3（パート単体の段割り・
上書き不適用・総譜復帰）を1本のシナリオとして固定。旧実装では
「上書きがパート譜にも効く」「段割りが選択パートに依存しない」の2点で落ちる。
