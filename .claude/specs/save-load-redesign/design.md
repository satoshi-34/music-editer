...
### 続報: 開くボタンのラベルを「MusicXML (.mxl)」へ（2026-08-29 対面テスト指摘）

.mxl 対応（#465/#467）後もボタンが「MusicXML」のままで、Finale 既定の圧縮形式が
開けることが画面から伝わらなかった。ラベルへ拡張子を併記し、title 属性に対応形式
（.musicxml / .xml / .mxl）の説明を追加した。配線テスト（ScorePageFileOpenWiring）は
新ラベルと title 属性の内容まで固定している。

## 圧縮MusicXML（.mxl）の読み込み対応（#465・2026-08-28）

Finale の既定書き出しは .mxl（ZIP コンテナ）で、対面テスト（2026-08-28）でユーザーの実ファイルが
開けなかった実害を受けて対応した。
