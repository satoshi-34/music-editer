...
| 10 | 五線の1間。広げすぎ防止の上限（これ以上は離さない。arcNoteheadClearance.test.ts で固定） |

Issue の指示「まず定数で +2〜3px 程度から調整」の範囲内で、
**符頭に触れなくなる最小値**を選んだ（+2 の 5 では、端の線の太さのぶんまだ接する）。

### 隙間は手動調整の有無にかかわらず一律（レビュー裁定・round1）

当初は「手動オフセット（startDx/Dy・endDx/Dy）がある端点だけ従来値 3 を使う」分岐を
入れたが、**未調整の端点を初めてドラッグした瞬間に基準が 6→3 へ切り替わり、
確定時に 3px 跳ねる不連続**が生まれる（横方向だけのドラッグでも Y が跳ねる）。
基準のバージョンを端点ごとに保存する案は複雑さに見合わないため、分岐を廃止して
**一律 ARC_NOTEHEAD_GAP = 6** とした。既存の手動オフセットは新基準からの相対値として
扱われ、旧データの端点は一律 3 外へ出る（相対的な調整の意図は保たれ、必要なら
ハンドルで再調整できる）。線間の音符では端点が最寄りの線をわずかに越え得るが、
浄書上許容とする（テストの文言も「1間以上離れない」ことだけを保証する形へ修正）。
...
- 段またぎの2セグメント・段の右端で切れるセグメント・終点側 Canvas の第2セグメントも
  同じ隙間（一律 6）。呼び出しは全4か所とも resolveArcEndpointY を通る
