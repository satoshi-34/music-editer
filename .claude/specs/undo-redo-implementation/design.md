...
### 追補2（2026-08-13・Codex レビュー2巡目）: 内容追跡の廃止と「外部差し替え限定・安全解除」への方針転換

追補1の「内容（JSON）で探し直す」方式には、なお2つのデータ破壊経路が残っていた:
1. 同一内容のイベント（[C,C,G] の C 同士）は区別できず、残った同音へ選択が乗り移る
2. 内部編集（休符分割等）が同じレンダーで設定した「新しい選択」を、
   旧 index からの追跡と誤解して別の音符へ移す

イベントに安定 ID が無い以上、内容からの同定は原理的に曖昧なので、追跡そのものをやめた:
- 親→子同期（唯一の外部入口）が実際に差し替えたときだけ `externalReplaceRef` を立て、
  整合処理は**外部差し替えのときしか動かない**（内部編集はハンドラが選択を管理する）
- 外部差し替えで選択中の小節・声部のイベント列が1つでも変わっていたら**選択解除**。
  変わっていなければ（他の小節だけの Undo 等）選択を保つ
- 描画側の keyIndex 範囲ガード（初版）は最終防衛としてそのまま残す

回帰テスト: [C,C,G] の同音乗り移り・休符分割直後の Delete で A が無傷・
他小節だけの差し替えでは選択維持、の3本を追加（計7本）。

### 追補3（2026-08-15・Issue #265）: 弧（スラー/タイ）と松葉の残存選択

#### 問題

追補2で音符の選択（`selected`）は「外部差し替え限定・安全解除」で守られたが、
**弧の選択（`selectedArc`）と松葉の選択（`selectedHairpin`）は同じ整合処理に載っていなかった**。

どちらも `{partIndex, voiceIndex, fromMeasure, fromEvent, arcIndex}` という
「何番目か」だけの index 参照で、弧の実体は始点イベントの `arcs[] / hairpins[]` の中にある。
そのため、1本目の弧を選んだあとに Undo でその1本目が消えると、

- 2本目の弧が `arcIndex = 0` へ繰り上がる
- 選択は `arcIndex = 0` のまま残る
- 次の `Delete` が、ユーザーの選んでいない2本目の弧を消す

という無言のデータ破壊が起きる（#238・#257 と同根の「残存選択」）。

#### 修正設計

追補2で作った整合 effect（`PianoSystemCanvas.tsx` 親データ同期の直後）に、
音符とまったく同じ規則で弧・松葉の選択も載せる。

- 判定するのは**外部差し替え（`externalReplaceRef` が立った同期）のときだけ**。
  内部編集（弧のドラッグ確定・Delete 自身）は各ハンドラが選択を管理しているので触らない
- 判定の材料は「**弧が載っている小節・声部のイベント列（`getVoiceEvents`）が変わったか**」の一点。
  弧の実体は始点イベントの中に入っているので、その列が1バイトも変わっていなければ、
  同じ index には必ず同じ弧が居る＝選択を保って安全。変わっていたら解除する
- 小節ごと消えた場合と、比べる相手（前回スナップショット）が無い場合は解除に倒す。
  音符と違い弧の選択は再クリックが軽い操作なので、ここは厳しめでよい（Issue #265 トリアージの裁定）

弧の終点側の小節が変わったケースは判定に含めない。終点が変わっても
「選択が指す弧の実体」は変わらないため、`Delete` は正しい弧を消す（宙に浮いた弧の掃除は
`.claude/specs/voice2-arc-support/design.md` 段2 の削除追従の担当で、本件のスコープ外）。

#### 影響範囲

- `src/components/PianoSystemCanvas.tsx`（整合 effect に `setSelectedArc` / `setSelectedHairpin` の掃除を追加）
- `src/components/PianoSystemCanvasStaleArcSelectionAfterUndo.test.tsx`（新規・回帰テスト3本）
  - 1本目の弧を選択中に Undo でその弧が消えると、繰り上がった2本目へ選択が乗り移らない（修正前は失敗）
  - 選択と無関係の小節だけの差し替えでは選択が保たれ、`Delete` が選んだ弧だけを消す
  - 松葉でも同じ（修正前は失敗）

ユーザーから見える操作は変わらないため README は更新していない。
