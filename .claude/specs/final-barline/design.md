...
## 追補3: 最終ページの「下端寄せ」を廃止し、他ページと同じ間隔の上詰めにした（Issue #506、2026-08-31）

### 症状

- 運用者の実機確認（印刷プレビュー）: 97小節のピアノ譜（15ページ）の**最終ページだけ**、段（システム）
  どうしの縦の間隔が他ページより大きく広がり、最下段の譜面がページ番号（`.page-foot`）と重なる。

### 原因

本ドキュメント「2. 最終ページの最終段をページ下端へ寄せる（印刷時のみ）」と追補2で入れた
最終ページ専用の配置がそのまま症状になっていた。最終ページは全ページ共通の行グリッド
（`--page-capacity` ベースの固定スロット高。`.claude/specs/page-layout-controls/design.md` 参照）を
`flex: 1 1 0% !important` で無効化し、`justify-content: space-between` で「残った段をページ全高へ
均等に散らす」配置にしていた。段数が端数になる最終ページでは、残り2〜3段がページの高さいっぱいへ
引き伸ばされるため段間隔だけが異常に広がり、最下段がページ下端＝ページ番号の領域へ食い込む。

### 修正設計

- 最終ページ専用の上書き（`flex` の無効化・`space-between`・1段だけのときの例外
  `print-final-page-single`）を **すべて削除**し、最終ページも他ページとまったく同じ行グリッドで
  配置する。段数が足りないぶんはページ下端の余白として残る（＝上詰め）。
- `.print-final-page .system-stack { justify-content: flex-start; }` は固定スロット高では
  既定値と同じ振る舞いだが、「意図して上詰めにしている」という宣言として明示的に残す
  （将来また `space-between` を足し戻さないための目印。`src/AppCssFinalPageTopAlign.test.ts` が見張る）。
- `@media print` と `.print-preview` の両方へ同じ扱いを入れる（プレビューと実際の印刷が食い違わないため）。
- `ScorePage.tsx` からは `print-final-page-single` の付与と、そのための
  `finalContentPageVisibleSystems` の算出を削除した（下端寄せをやめたことで、
  「1段だけのときは下端へ落とさない」という例外自体が不要になったため）。
  `print-final-page` クラス自体は「内容のある最後のページ」の目印として残している。

### 過去の設計との関係（重要）

本ドキュメント §2・追補2 の「終止線がページ右下角で締まるよう最終段をページ下端へ寄せる」という
設計判断は、この追補で**撤回**された。撤回はリポジトリオーナーが Issue #506 のトリアージコメントで
明示的に決めたもので、根拠は次のとおり:

- 運用者の期待（および市販譜の作法）は「最終ページは上詰め・下は余白」である
- 元ファイル（Finale 書き出し）の実測でも、`<print><system-layout><system-distance>` は
  最終ページで多少広げる（実測例: 標準137に対し211/179/167）だけで上限を守り、
  下余白＝ページ番号の領域は必ず残す「制限付きの引き伸ばし」だった。ページいっぱいの均等割りはしていない
- Finale 由来ファイルの「Finale が決めた段間隔の再現」は #477 段階2（段替え・間隔の引き継ぎ）の
  守備範囲であり、本Issueの自前レイアウトの既定値とは別問題として扱う

### 影響範囲

- `src/App.css`: `@media print` と `.print-preview` の最終ページ用ルールを、上記の1行ずつへ置き換え。
- `src/components/ScorePage.tsx`: `print-final-page-single` の付与と `finalContentPageVisibleSystems` を削除。
- `src/AppCssFinalPageTopAlign.test.ts`（新規）: 最終ページ用ルールに `space-between` 等の
  下端寄せ・均等割りと、行グリッドを無効化する `flex` の上書きが復活していないことを静的に検査。
- `src/AppCssPrintPreviewHiddenSystems.test.ts`: 旧仕様（`space-between` / `-single` の上揃え）を
  前提にしていた1件を、新仕様（上詰め）の確認へ更新。
- 満杯のページ（＝最終ページ以外のすべて）のレイアウトは無変更。最終ページが**ちょうど満杯**の
  場合だけは「実サイズ＋space-between」から他ページと同じ固定スロット配置へ変わるが、
  これは他ページと揃える方向の変化であり意図どおり。
