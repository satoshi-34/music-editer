...
パート編集は別ウィンドウでも開けるが、ポップアップ制限や in-app browser では
別ウィンドウが見えず、「ユーザーが編成を決める機能が無い」ように見える。
そのため `ScorePage` の楽譜設定タブ内にインライン編集パネルを表示する。
...
- パート順の上下移動
- パート名・略称
- 楽器族・音部記号・移調・括弧グループ・サブ括弧
- パート別の再生音色

実装方針:

- 既存の `updateInstrumentationParts()` をそのまま使い、編成定義と
  `ensembleParts` の小節データ配列をパート ID で同期する。
- 編集した時点で `presetId='custom'`、`name='カスタム編成'` にする。
- 画面内パネルを第一導線にし、広い編集が必要な場合だけ「別窓」ボタンで
  既存の Portal ウィンドウを開けるようにする。
- 初心者が追えるように、編集行の JSX には「編成定義と小節データを同時に
  そろえる場所」であることをコメントとして残す。
