...
- **P1 最下段以外のペダル**: 段の下余白の予約は最下段ぶんしか無いので、最下段以外（下に別の
  五線がある）のペダルは `resolvePedalBaselineY` の `maxBaselineY`（下の五線の上端 − 3px −
  ディセント）で**境界停止**する（強弱記号の #382 と同じ流儀）。隣の五線へは食い込まず、
  段の高さも変わらない。見積もり（`estimatePedalBottomExtensionPx`）が最下段だけを見るのは
  この前提による
- **P1 ScorePage と canvas の入力の一致**: パート譜表示中は canvas に渡る段も選択パートだけ
  なので、ScorePage 側も選択パート（大譜表なら2段）だけで見積もる。関数の引数は
  「画面に描く段の並び」と定義した
- テスト: 純関数に境界停止の3ケース、canvas に「右手の Ped が左手の五線の手前で止まり段の高さが
  変わらない」を追加
