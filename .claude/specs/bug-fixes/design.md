...
// ヒット領域: 固定窓（-3〜7）と「符頭の線 ± 1」の広いほう。狭くなることはない。
const hitTopLine = Math.min(CHORD_LEDGER_TOP, keyLines.minLine - 1);
const hitBotLine = Math.max(CHORD_LEDGER_BOT, keyLines.maxLine + 1);
// 選択の丸め先: 固定範囲（-4〜10）と「符頭の線ちょうど」の広いほう（±1 はしない。理由は下記5）
```
...
3. **新規音の音高（`newKey`）は従来どおり `snapLine()`（line -4〜10）から作る**。
   ...
   置ける音域は変わらない
4. **広げたぶんは隣のパートとの中間線でクリップする**。上の「段間クリックの当たり判定バグ」で
   背景ヒットに入れたのと同じ分割線を使う。これが無いと、上のパートの遠い音符のヒット領域が
   下のパートへ覆いかぶさり、同じ誤配置（上のパートがクリックを奪う）を再発させてしまう。
   `Math.min(chordTopY, Math.max(広げた上端, 中間線))` の形にして、**固定範囲より狭くはならない**ようにしている
5. **選択の丸め先を広げるのは「符頭の線ちょうど」まで**（ヒット領域のような ±1 はしない）。
...
また、判定窓を広げたことで「音符を置ける範囲（line -4〜10）の外」までクリックが rect に届くようになるため、
選択にならなかったクリックのうちその範囲外のものは **何もしない**（従来は届かなかった位置なので、
挿入へ回すと丸め先が範囲端に張り付いて意図しない音高の音符が増えてしまう）。
...
- `src/components/PianoSystemCanvas.tsx`
  ...
  - ヒット rect の縦範囲を符頭の位置から計算（隣パートとの中間線でクリップ）
...
- `src/components/PianoSystemCanvasOuterLedgerSelect.test.tsx`（新規・回帰テスト6件）
...
- 大譜表で上パートの遠い音符のヒット領域が、下パートの五線へ食い込まないこと
