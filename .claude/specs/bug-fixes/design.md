...
段と段の「間」をクリックすると、上段に置きたい低音が下段側に
**極端な上加線の音**として入ってしまう。運用者の実機テスト（月光の入力）で
「右手の低い音を置こうとしたら左手に入った」として報告された。
入った音符は五線から遠いので選択もしづらく（Issue #218）、
2つ重なると「消せない謎の音符」に見える。

### 原因

過去の「段間クリックの当たり判定バグ」（本ファイル上方の追記）では、
**小節の背景 `rect.vf-hit` だけ**を隣パートとの中間線でクリップした。
一方、音符ごとの当たり判定 `rect.vf-note-hit` は
**「五線 ± 3加線」の固定範囲（`CHORD_LEDGER_TOP = -3` / `CHORD_LEDGER_BOT = 7`）のまま**だった。

SVG は後から追加した要素が手前に来る。パートは `pi` の昇順に描かれるので、
**下のパートの `.vf-note-hit` が最後に置かれ、上のパートの守備範囲を奪う**。

### どの譜面で起きるか（パート間隔に依存する）

五線の高さは 40（line0〜line4）、加線3本ぶんは 30。
中間線は「自分の五線の下端と次パートの五線上端の中間」なので、
パート間隔を `S` とすると中間線までの余白は `(S - 40) / 2`。
**`(S - 40) / 2 < 30`、つまり `S < 100` のとき**に `.vf-note-hit` が中間線を越える。

| 譜種 | パート間隔 `S` | 中間線までの余白 | 重なり |
| --- | --- | --- | --- |
| ピアノ（**既定**: 自動値80＋オフセット38＝118, Issue #199） | 118 | 39 | **なし** |
| ピアノ（オフセット0。Issue #199 より前に保存した譜面／スライダーを詰めた場合） | 80 | 20 | **10ぶん重なる** |
| 弦楽四重奏（オフセット既定0） | 80 | 20 | **10ぶん重なる** |
| 編成譜（5パート以上・オフセット既定0） | 60 | 10 | **20ぶん重なる** |

つまり**ピアノの既定値のままなら再現しない**。実測でも、既定のピアノ大譜表
（ト音 line0 = 60 / ヘ音 line0 = 178）では `.vf-note-hit` が 30〜130 と 148〜248 で、
中間線 139 をどちらもまたいでいなかった。
再現するのは上表の「重なる」行、すなわち**四重奏・編成譜・パート間隔を詰めたピアノ譜**である。

パート間隔80のピアノ大譜表での実測（修正前）:

| 要素 | Y範囲（VexFlow内部座標） |
| --- | --- |
| ト音の五線 | line0 = 60 / line4 = 100 |
| ヘ音の五線 | line0 = 140 / line4 = 180 |
| 中間線（`.vf-hit` の境界） | **120** |
| ト音の `.vf-note-hit` | 30 〜 130 |
| ヘ音の `.vf-note-hit` | **110** 〜 210 |

`elementFromPoint` で 2.5 刻みに走査すると、帰属は中間線 120 ではなく
**110 で切り替わっていた**。この帯（110〜120）はヘ音では line -3 〜 -2、
つまり**上加線3本相当**にあたり、症状の「極端な上加線で入る」と一致する。

### 修正設計

`.vf-note-hit` も `.vf-hit` と同じ中間線（`staveTop` / `staveBot`）でクリップする。

```typescript
// src/components/PianoSystemCanvas.tsx（音符ヒット領域の縦範囲）
const eventKeyLines=(activeEvs[j]?.keys??[]).map(k2l);
const headTopY=eventKeyLines.length ? stave.getYForLine(Math.min(...eventKeyLines)-0.5) : chordBotY;
const headBotY=eventKeyLines.length ? stave.getYForLine(Math.max(...eventKeyLines)+0.5) : chordTopY;
...
**符頭そのものが中間線の外に描かれている場合だけは、その符頭のぶん（0.5ライン）を残す。**
単純にクリップすると、境界の外に描かれている音符（例: ヘ音の g/4 = line -3）の符頭を
押しても選択できなくなり、Issue #218 と同じ症状を新たに作り込んでしまうため。
残すのは「符頭が実際に描かれている高さ」だけなので、
**音符が何も無い空白を隣のパートから奪うことはもう起きない**。

パート間隔が広い譜面（ピアノ既定の118など）では `staveTop`/`staveBot` が
和音ゾーンの外側にあるため、**この式は従来と1pxも変わらない**。

### ブラウザ実測（修正前 / 修正後・同一譜面・同一クリック位置）

worktree だけを載せた使い捨ての dev サーバーで、**パート間隔80のピアノ大譜表**
（＝重なりが起きる条件）に対し、同じ点（内部座標 x = 640 / **y = 115**、
＝中間線 120 より上＝ト音側）をクリックした結果:

| | クリックを受け取った要素 | 挿入先 | 入った音 |
| --- | --- | --- | --- |
| 修正前 | ヘ音の `.vf-note-hit`（y=110） | **左手（ヘ音）** | **f/4**（ヘ音の上加線2〜3本） |
| 修正後 | ト音の `.vf-note-hit`（y=30） | **右手（ト音）** | **b/3**（ト音の下第1〜2加線） |

修正後の切り替わり位置は `elementFromPoint` で **ちょうど 120（中間線）** になった。
ガイド（ゴースト表示）も帰属先の段を正しく示す:

| クリックY | 帰属 | ガイドの点 | ガイドの加線 |
| --- | --- | --- | --- |
| 119 | ト音 | 120 | 110・120（**ト音の五線より下**＝下加線2本） |
| 121 | ヘ音 | 120 | 120・130（**ヘ音の五線より上**＝上加線2本） |

同じ高さでも加線が伸びる向きが違うので、どちらの段に入るかは目で見て分かる。

既定のピアノ大譜表（パート間隔118・調号♯4つ＝cis moll）でも、実際に音符を入れて確認した。
切り替わりは中間線 139 ちょうどで、段間クリックの結果は次のとおり:

| クリックY | 帰属 | 実際に入った音 | ガイドの加線 |
| --- | --- | --- | --- |
| 125（ト音 line 6.5） | 上段 | **右手 g#/3** | 110・120（ト音の下2本） |
| 141（中間線のすぐ下） | 下段 | （ヘ音 line -3.9 相当） | 138〜168（ヘ音の上4本＝極端な上加線が見えている） |
| 158（ヘ音 line -2） | 下段 | **左手 e/4** | 158・168（ヘ音の上2本） |
| 168（ヘ音 line -1） | 下段 | **左手 c#/4** | 168（ヘ音の上1本） |

つまり既定値では、月光の右手 g#/3 は段間クリックで右手に入り、
ヘ音側の上加線1〜2本も従来どおり左手に置ける。
中間線のすぐ下では加線が4本並んだガイドが出るので、
「このまま押すと下段の極端な上加線になる」ことがクリック前に分かる。
コンソールエラーは0件。

### 影響範囲

- `src/components/PianoSystemCanvas.tsx`: `.vf-note-hit` の縦範囲のみ変更＋`data-line0-y` 属性を追加
  （X範囲・和音判定 `isOnNote`・音高スナップ `snapLine`・`.vf-hit` は無変更）
- `src/components/PianoSystemCanvasPartGapClick.test.tsx`（新規・6件）
- `src/components/PianoSystemCanvasPartSpacing.test.tsx`: パート間隔の測り方を
  rect の `y` から `data-line0-y` へ変更（下記）
- パートが1つだけの譜面（単旋律）は `staveTop` / `staveBot` がクリップされないため**完全に不変**

#### `data-line0-y` を足した理由

`PianoSystemCanvasPartSpacing.test.tsx`（Issue #29 / #90）は
「`.vf-note-hit` の `y` の差＝パート間隔」という前提でパート間隔の均一性を検証していた。
本修正でヒット領域の上端が中間線までクリップされるようになったため、
`y` の差はパート間隔と一致しなくなる（先頭パートだけクリップされないので差が不揃いになる）。
クリップの影響を受けない `stave.getYForLine(0)` を `data-line0-y` として公開し、
テストはそちらを見るようにした。パート間隔そのものの計算・描画は変えていない。

### 確認ポイント

- 弦楽四重奏・編成譜、およびパート間隔を詰めたピアノ譜で、
  五線と五線の中間より上をクリックすると必ず上の段に入ること
- 下の段の上加線1〜2本（line -1 / -2）は従来どおり下の段に置けること
- 五線内の音符の選択・和音追加・休符置換が変わらないこと
- 境界の外に符頭が描かれている音符（例: ヘ音の g/4）が、従来どおり符頭クリックで選択できること
- ピアノ既定値（パート間隔118）の譜面で、当たり判定の座標が修正前と1pxも変わらないこと

### 残っている制約（運用者判断が要る点）

「中点で分割」という方針そのものの帰結として、**パート間隔が狭い譜面では
上の段が受け取れる加線の本数が減る**。上の段が下加線 n 本ぶんまで受け取るには
パート間隔が `40 + 10n` より広い必要がある（例: 下第2加線＝A3 まで欲しいなら 60 以上、
その下の g#/3 まで欲しいなら 90 以上）。

- ピアノ既定（118）: g#/3 まで上段で置ける（実測済み）
- 弦楽四重奏・オフセット0のピアノ（80）: 上段が置けるのは**下第2加線まで**。
  その下は下の段に入る（実測で d/4 になった）
- 編成譜（60）: 上段が置けるのは下第1加線まで

狭い譜面で「もっと下」を上段に置きたい場合の回避策は2つ。
いったん置いてから **↑/↓キーで音高を動かす**（既存機能）か、
**「パート間隔」スライダーを広げる**。
恒久策として「クリック時に修飾キーで帰属先の段を明示的に選ぶ」も考えられるが、
本Issueのスコープ外なので実装していない。
