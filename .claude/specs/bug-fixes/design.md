...
## スラー当たり判定の音符クリック吸収と、固定範囲外の極細選択帯の修正（2026-08-12）

### 問題（実機テストで発見・2つの合わせ技）

スラーの掛かった低音の三連符（固定範囲＝五線±3加線の外）が実質クリック不能だった。

1. **スラー/タイの当たり判定が弧パス全体**（透明 stroke 10px）だったため、端点付近の帯が
   符頭に重なり、音符を狙ったクリックがスラー選択に化けていた。
2. **固定範囲の外の音符は「線ちょうど（±0.25ライン＝100%ズームで約2.5px）」しか選択にならない**。
   あの帯は挿入も和音追加もしない設計（隣パートとの重なり対策・#218）なので、
   外すと完全な無反応になり、1 の帯とちょうど重なって「何をしても選択できない」状態だった。

### 修正設計

1. **弧の当たり判定を中央部（頂点まわり）だけに限定**（Finale 等の浄書ソフトと同じ方針）。
   `arcUtils.ts` に `computeArcHitGeometry` を新設し、表示用と同じ制御点計算から
   ベジェ曲線の t∈[0.25, 0.75] 区間を de Casteljau 分割で切り出す。
   `PianoSystemCanvas.tsx` の弧描画（drawArcPathP）とドラッグ中の d 更新3箇所で使用。
   端点の操作は選択後に出る丸ハンドルが担うので機能は失われない。
2. **固定範囲の外に限り、最寄りの構成音へ±1ライン以内で吸い寄せ**る
   （`findNearestKeyIndexWithinLines` を新設）。クリックとホバー（cursor='pointer' 判定）の
   両方に同じ式で適用。固定範囲の中の挙動（外したら和音追加/挿入）は一切変えない。
   吸い寄せが効く帯はもともと「何も起きない」帯なので、音符の追加位置がずれる副作用はない。

### 影響範囲

- `src/components/arcUtils.ts`: `computeArcHitGeometry` 追加（純関数）
- `src/components/PianoSystemCanvas.tsx`: 弧ヒットパス生成1箇所＋ドラッグ更新3箇所、
  クリックの個別音選択とホバー判定に吸い寄せを追加
- `src/components/arcHitGeometry.test.ts`（新規4本）: 切り出しの幾何を固定
- `src/components/PianoSystemCanvasOuterNoteSnapSelect.test.tsx`（新規3本）:
  吸い寄せの成立・非成立と、五線内の従来挙動の保全を固定

### 動作確認（ブラウザ）

1. スラー端点の符頭クリック → 音符が選択される（従来はスラー選択に化けた）。
2. スラー頂点付近のクリック → 従来どおりスラーが選択できる。
3. 固定範囲外の低音の選択可能帯が実測 2〜3px → 約8px以上へ拡大。
4. コンソールエラーなし。`docker compose run --rm app npx vitest run` 全 1565 件パス。
...
多段譜（大譜表）で、上段・声部2の段間へ深くはみ出した符頭をクリックすると、
音符の選択ではなく下段（左手）への操作になった（実機テスト・検証用譜例5小節目）。

原因は当たり判定の重なり順。小節背景（.vf-hit）は段間クリック帰属（Issue #219）のため
隣段側の帯まで覆っており、SVG の「後に描いた要素が前面」の規則で、
後から描かれる下段の背景が上段の音符ヒット領域（.vf-note-hit）を覆い隠していた。

### 修正設計

全パート描画後に `.vf-note-hit` を `svgRoot` の末尾へ移して前面に出す（1行）。
- 透明な当たり判定要素の並べ替えなので見た目は変わらない
- この後に描く記号類の当たり判定（symbol-hit-region 等）はさらに前面に積まれるため、
  記号クリックの優先は従来どおり
- 副作用として、弧の当たり判定（vf-arc-hit・ループ内で描画）よりも音符が前面になるが、
  弧は中央部限定（computeArcHitGeometry）にした後なので実害はなく、
  「符頭の上では音符が勝つ」という方針にはむしろ合致する

> **追記（2026-08-15・PR #290 レビュー時の実測）**: 上の「弧より音符が前面」は
> **現状の実態と異なる**。この記述の後、弧の描画（＝vf-arc-hit の追加）が
> `.vf-note-hit` を末尾へ移す並べ替えより**後**に移ったため、いまは重なった場所で
> `elementsFromPoint` が**弧を先に返す**（＝1回目のクリックは弧が勝つ）。
> ただし #264（再クリック巡回）が入ったことで、どちらが前面でも**2クリック目で
> 必ずもう一方に届く**ようになり、この順序は実害を持たなくなった。
> そのため並べ替えの再修正は行わず、実態をここに記録するに留める
> （経緯と裁定は `.claude/specs/click-target-cycling/design.md` と PR #290 の申し送り参照）。

### 影響範囲

- `src/components/PianoSystemCanvas.tsx`（並べ替え1行）
- `src/components/PianoSystemCanvasNoteHitZOrder.test.tsx`（新規1本。
  全 note-hit が全背景より DOM 順で後ろにあることを固定。修正前は失敗することを確認済み）
...
## 五線内でも既存の構成音を和音追加より優先して選択する（2026-08-15, Issue #271）

### 問題（実機テスト・月光2小節目）

2度でぶつかる和音 `[e/4, f#/4]` の上の音（符幹の右へずらして描かれる F#）が実質選択できない。

五線の**中**で個別選択が成立するのは、クリックYを `snapLine()` で 0.5 ライン刻みへ丸めた結果が
その音の線と一致したときだけ ＝ 実質「線ちょうど ±0.25 ライン」（100%ズームで実測 ±2.4px）。
そこを外すと**和音追加**として解釈され、調号適用で `G→G#` のように
「何が起きたのか分かりにくい音」が増えていた。

#255（本ファイルの「スラー当たり判定の音符クリック吸収と、固定範囲外の極細選択帯の修正」節）で
入れた吸い寄せは**固定範囲＝五線±3加線の外だけ**が対象で、五線内は手つかずだった。

### 修正設計（運用者裁定: 案A・選択優先）

**符頭のX範囲内のクリックは、既存の構成音の ±0.3 ライン以内なら和音追加より個別選択を優先する。**

裁定の理由（運用者）: 和音追加には「離れた所に置いて矢印キーで近づける」という回避手段があるが、
選択できない場合には回避手段が無い。

実装は `PianoSystemCanvas.tsx` の描画ループ内に**判定式を1本化**する形で入れた。

1. `INNER_KEY_SELECT_MAX_LINES = 0.3` を新設（`OUTER_KEY_SELECT_MAX_LINES = 1.0` の隣）。
   **ライン単位**で持つのが要点で、五線が拡大縮小しても「見た目に対する許容幅」が一定に保たれる
   （raw 単位の定数にすると編成譜や画面ズームで画面px換算の幅がズレる。`keySelectXPad` と同じ考え方）。
   0.5 未満にしてあるのは、隣の線／間（＝0.5 ライン離れた位置）を狙ったクリックが吸い寄せられて
   和音追加できなくならないようにするため。
2. `resolveSelectableKeyIndexAt(lx, ly)` を新設し、**「押したら個別音選択になるか」の判定をこの1本に集約**した。
   分岐は上から順に「符頭X範囲（`±keySelectXPad`）の外なら -1」→「線ちょうど一致（`findKeyIndexAtLine`）」
   → 「固定範囲の外なら ±1 ライン吸い寄せ（#255・従来どおり）」→ **「五線内かつ符頭のX範囲内なら ±0.3 ライン吸い寄せ（本修正）」**。
3. 五線内の吸い寄せのX範囲は、下の `isOnNote`（和音追加ゾーン）と**まったく同じ式**
   （`noteVisualLeft-CHORD_HIT_PAD` 〜 `noteVisualRight+CHORD_HIT_PAD`）にしてある。
   「和音追加になり得る場所でだけ選択が先に立つ」形なので、符頭から横に離れた位置の
   挿入・和音追加の挙動は1pxも変わらない。

### なぜ判定式を1本化したか

従来は同じ式が3箇所（click の `clickedKeyIndex` / mousemove の `wouldSelectKey` /
再クリック巡回の `resolveSelectableKeyIndex`）に**書き写されて**いた。
条件を1つ足すたびに3箇所そろえる必要があり、ずれると
「ホバーでは選択に見えるのに押すと和音追加になる」「巡回では選べるのに普通に押すと選べない」
という、原因の見えない食い違いが出る（過去に何度も踏んでいる轍）。
本修正では3箇所とも `resolveSelectableKeyIndexAt` を呼ぶだけにして、構造的に食い違えないようにした。

### 影響範囲

- `src/components/PianoSystemCanvas.tsx`
  - `INNER_KEY_SELECT_MAX_LINES` 追加
  - `resolveSelectableKeyIndex`（画面座標版）を `resolveSelectableKeyIndexAt`（SVG内部座標版）＋薄い変換に分割
  - mousemove の `wouldSelectKey`・click の `clickedKeyIndex` を同関数の呼び出しへ置換
- `src/components/PianoSystemCanvasInnerNoteSelectPriority.test.tsx`（新規7本）
- **単旋律譜（旧 `StaffCanvas`）は対応不要**。`StaffCanvas.tsx` は
  `docs/phase2-staffcanvas-retirement-feasibility.md` の手順3で削除済みで、
  単旋律譜は `SingleStaff` → `PianoSystemCanvas` を通る。よって本修正がそのまま効く。

### 変えていないこと

- 固定範囲の外の ±1 ライン吸い寄せ（#255）
- 符頭のX範囲外（`CHORD_HIT_PAD` の外〜`keySelectXPad` の内）の挙動
- 既存音から 0.5 ライン離れた位置のクリック（従来どおり和音追加）
- 保存データ・描画・レイアウト

### 動作確認

自動テスト（新規7本。うち4本は本修正を外すと落ちることを `INNER_KEY_SELECT_MAX_LINES = 0` で確認済み）:

- 2度の和音 `[e/4, f#/4]` で、上の音を 0.3 ライン外したクリックでも上の音が選択される
- 同じ和音で下の音側を外したクリックでは下の音が選択される（近いほうが勝つ）
- 単音でも 0.28 ライン外したクリックが選択になる（旧: 隣の間へ和音追加）
- 0.35 ライン離れたクリックは従来どおり和音追加（選択優先が効きすぎない）
- 符頭のX範囲の外ではこの吸い寄せが働かない（ホバーのカーソルが `copy`）
- ホバーのカーソル形状がクリック結果と一致する

ブラウザ（worktree だけを載せた使い捨て dev サーバー・Chromium）:

| 操作（2度の和音 `[e/4(line4), f/4(line3.5)]`・符頭中央X） | 結果 |
| --- | --- |
| line 3.26（丸め先は line 3.0）をクリック | **上の音（line 3.5）が選択**。符頭は `4,3.5` のまま増えない |
| line 4.26（丸め先は line 4.5）をクリック | **下の音（line 4）が選択**。符頭は増えない |
| line 3.0（0.5 ライン上）をクリック | 従来どおり **`g/4` が和音追加**（符頭が `4,3.5,3` へ） |
