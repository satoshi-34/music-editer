...
## 追記: 非アクティブ声部の音符クリックを「切り替えて選択」にする（Issue #258, 2026-08-17）

### 問題

Issue #105（上記の追記）で「当たり判定はアクティブ声部にしか作らない」設計にした結果、
**非アクティブ声部の音符をクリックしても無言で何も起きない**状態になっていた。
単声部の小節では非アクティブ声部の音符も黒色で描かれるため、ユーザーからは
「選択できるはずなのに反応しない」ように見える。2026-08-12 の実機テストで運用者が
「右手の下声が触れない」として報告し、#295（五線から離れた声部2の低音が選べない）も
同じ症状の一例だった。

### 仕様変更（運用者裁定 2026-08-16）

「判定はアクティブ声部のみ」から、**「選択のクリックは全声部・編集の入力はアクティブ声部のみ」**へ
**意図的に仕様変更**した。#105 の目的（気づかない誤編集を防ぐ）は、声部が切り替わったことを
**必ず通知する**ことで引き継ぐ。黙って声部が変わることはないので、誤編集には気づける。

1. 既存の音符（符頭）をクリックしたとき、それが非アクティブ声部のものなら、
   その声部へ切り替えたうえでその符頭を選択し、「声部Nに切り替えました」を通知する
2. 音符以外（空白・挿入ゾーン・小節背景）のクリックは従来どおり**アクティブ声部への入力**。
   切り替わるのは「既存の符頭を掴んだとき」だけ
3. 重なった場所では再クリック巡回（#264）の候補に他声部の符頭も載る
4. 単声部の譜面・同一声部内のクリックは1pxも変えない

### 修正設計

**判定式の共用（`buildNoteHitGeometry`）**

アクティブ声部の当たり判定を作っていたコードのうち、幾何計算と
「その位置は符頭を選ぶクリックか」の判定（`resolveSelectableKeyIndexAt`・#297）を、
声部を知らない関数 `buildNoteHitGeometry(evs, vfNotes, j, anchors, mids)` として
`PianoSystemCanvas.tsx` から切り出した。非アクティブ声部の符頭も同じ関数で判定する。
声部ごとに似た判定式を2つ持つと、片方だけ直したときに食い違う（#280 で実際に起きた
「同じロジックの2枚目」の事故）ため、共用を第一候補にするという方針に従っている。

**選択専用の当たり判定（`.vf-inactive-voice-note-hit`）**

非アクティブ声部の符頭には、選択専用の透明 rect を新しく作る。守っている条件は3つ:

- **大きさは符頭1つぶんだけ**（X = 符頭の実描画範囲 ± `keySelectXPad`、Y = その線 ±0.5ライン）。
  和音は符頭が縦に離れて並ぶので、符頭ごとに1枚ずつ作る。アクティブ声部のような
  セル全幅の rect にすると、符頭から離れた場所のクリックまで飲み込み、
  アクティブ声部への新規入力（挿入ゾーン・小節背景）が効かない帯ができてしまう
- **アクティブ声部の rect より先に追加する**。SVG は後から追加した要素が手前になるので、
  重なった場所では必ずアクティブ声部が優先される（＝受入条件4を構造的に保証する）
- **ハンドラは選択・切り替え・通知・試聴だけ**。音符を増やす/置き換える編集は一切しない

クラス名をアクティブ声部の `.vf-note-hit` と分けているのは、`.vf-note-hit` を
「アクティブ声部の編集用ヒット領域」として参照している CSS・テストへ意味の違うものを
混ぜないため。

**声部の切り替え要求（`SCORE_ACTIVE_VOICE_CHANGE_EVENT`）**

声部の状態（`activeVoice`）を持っているのは `ScorePage` で、譜面とのあいだに5つの
ラッパーが挟まっている。props でコールバックを通すと5ファイルを機械的に書き換えることに
なるため、通知（`notifyScoreEdit`）と同じ window の CustomEvent 方式にそろえた
（`utils/scoreEditorNotices.ts` に `requestActiveVoiceChange` / `describeActiveVoiceSwitched` を追加）。
`ScorePage` はこのイベントを受けて `setActiveVoice` するだけでよい。

### 影響範囲

- `src/components/PianoSystemCanvas.tsx`: 幾何計算の関数化（挙動は不変）＋選択専用 rect の追加
- `src/utils/scoreEditorNotices.ts`: 声部切り替え要求のイベントと文言を追加
- `src/components/ScorePage.tsx`: 切り替え要求のリスナーを追加
- `src/components/PianoSystemCanvasVoiceClickScope.test.tsx`: #105 のテストを新仕様へ書き換え
  （編集用ヒット領域が作られないことは従来どおり／選択専用領域が作られることを追加）

### テスト

`PianoSystemCanvasVoiceClickScope.test.tsx`:

- 声部2アクティブ・声部2未入力の小節で、声部1の**編集用**ヒット領域が作られないこと（#105 から不変）
- 同じ小節で、声部1の符頭には**選択専用**の領域が作られること（新仕様）
- 背景クリックは従来どおり声部2へ追加されること（#105 から不変）
- 声部2アクティブのまま声部1の符頭をクリックすると、声部1への切り替え要求と通知が出て、
