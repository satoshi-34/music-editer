// テキストの書式（書体・文字サイズ・太さ）をその場で変えるための小さな共通部品（Issue #576）。
//
// タイトルの書式は元々「楽譜設定」タブに常設されていたが、使う頻度は「まれ」で、
// 常に見えていること自体が邪魔だった（運用者指示 2026-09-02）。編集する対象のそばで
// 触れる形へ移すにあたり、この3つのコントロールだけを **共通部品** として切り出してある。
...
/**
 * 書体・文字サイズ・太さの3つを並べた書式コントロール。
 * 呼び出し側（タイトル編集ダイアログ）の中に置かれる前提なので、枠や影は持たない。
 */
