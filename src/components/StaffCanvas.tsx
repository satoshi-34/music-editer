...
export default function StaffCanvas({
  ...
  useEffect(() => {
    ...
    // 歌詞は他の下側テキストと違い「五線の上」に表示する（PianoSystemCanvas と統一した仕様）ため、
    // 上側配置の基準になる五線上端 Y も一緒に持たせる
    const lyricsEntries: Array<BotTextEntry & { staveTopY: number }> = [];
        ...
        if (vfNotes.length > 0) {
          ...
          vfNotes.forEach((n: any, j: number) => {
            ...
            if (!safeEvents[j]?.__isPlaceholder) {
              ...
              if (ev?.lyrics) {
                lyricsEntries.push({ anchorX: cx, botY: staveBot, staveTopY: stave.getYForLine(0), text: ev.lyrics, adjust: getSymbolAdjust(ev, 'lyrics'), measureAbsoluteIndex: absoluteIndex, eventIndex: j, event: ev });
              }
              if (ev?.pedalMark) {
    ...
    // 描画した text 要素を受け取り、演奏記号タブでの直接クリック調整用のヒット領域を重ねる。
    // 歌詞はフェーズ2の仕様統一で、単旋律譜でも五線の上（placement: 'above'）に表示する
    lyricsEntries.forEach((entry) => {
      const el = drawLyricsEntry(svgRoot, { ...entry, placement: 'above' });
      appendSymbolHitRegion([el], entry.measureAbsoluteIndex, entry.eventIndex, entry.event, 'lyrics');
    });
