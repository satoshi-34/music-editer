...
/**
 * 単声部のままレンダリングしたときの弧のパス（現行実装の実測値）。
 *
 * 固定しているのは次の2つ:
 *   1. 単声部で描いたときのパス（現行実装の実測値）
 *   2. 声部2を全部削除したら、このパスへ**戻る**こと（Issue #305）
 * 多声部のままのケースは、むしろ単声部と一致しないことを別途検証している（同ファイル後半）。
 * つまり「多声部向けの変更が単声部へ漏れていないか」を数字で見張るための基準値。
 *
 * 曲率そのものを固定しているわけではないので、意図的に調整したときは更新する
 * （2026-08-24 に更新済み）。
 */
// 2026-08-24: スラーの曲率を緩めた（下限10→5px・係数0.15→0.09・上限24→16px・制御点0.25→0.32）ため、
// この基準値も更新した。固定している性質は「単声部と多声部で1文字も変わらない」ことであり、
// 曲率そのものではない
// 2026-08-29（#446）: 端点と符頭の隙間を 3 → 6 へ広げたため、両端のYが 3 ずつ外側
// （下向きの弧なので下）へ動いた。ふくらみ（制御点）は符頭を基準に決まるので変わらない。
// 2026-09-03（#559）: 小節の最低幅を「VexFlow の理想幅×圧縮率」へ変えたことで、段内の
// 小節幅の配分が変わり、3小節目の音符が約8px 左へ動いた（弧のY・形は変わっていない）。
// このテストが固定しているのは「単声部と多声部で1文字も変わらない」ことなので、
// 基準値だけを実測値へ更新した。
// 2026-09-04（#624）: 段割りの圧縮率 0.64 → 0.3 で小節幅の配分がまた変わり、x 座標が約11px 右へ。
// 圧縮率を変えたらこの基準値も一緒に更新する（弧のY・形は変わっていない）
const SINGLE_VOICE_ARC_D =
  'M 641.847 126 C 675.663 130.255 713.538 130.255 747.055 106 ' +
  'C 713.239 128.683 675.364 128.683 641.847 126 Z';
