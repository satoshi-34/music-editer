...
describe('未対応の強弱記号の読み込み通知（#552 round1 P2）', () => {
  ...
  it('sfz を含むファイルでも読み込みは成功し、未対応1件の通知が表示される', async () => {
    seedWork();
    render(<ScorePage />);
...
<score-partwise version="3.1">
  ...
  <part id="P1">
    ...
    fireEvent.change(input, { target: { files: [new File([xml], 'dyn.xml', { type: 'application/xml' })] } });

    // 読み込み成功（音符が描かれる）+ 通知に「未対応の強弱記号 1 件」が出る
    await waitFor(() => {
      const notice = screen.queryByTestId('edit-notice');
      expect(notice?.textContent).toContain('未対応の強弱記号 1 件');
    }, { timeout: 15000 });
