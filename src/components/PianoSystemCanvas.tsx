...
import { fillPriorMeasureRests } from '../utils/measureRestFillUtils';
...
import { MAX_VOICES_PER_PART, buildTrailingRestEventsForBeats, computeVoiceDisplayPadding, getEventDurationBeats, getMeasureDurationBeats, getMeasureVoices, getPrimaryVoiceEvents, getVoiceEvents, resolveVoiceStemDirections, withVoiceEventsUpdated } from '../utils/voiceMeasureUtils';
...
export default function PianoSystemCanvas({
  ...
  const clickCyclePendingRef = useRef<PendingClickCycle | null>(null);
        ...
        if(activeVfNotes.length>0){
          ...
          activeVfNotes.forEach((n:any,j)=>{
            ...
            hit.addEventListener('click',e=>{
              ...
              // 読む口（段6b-4e）: 譜面の最新ミラー・小節容量・設定。書かない側を noteWriter と分けて渡す
              const noteReader: NoteReader = { partsScoreRef, capacityBeatsAt, previewAccidentalOnApply };
