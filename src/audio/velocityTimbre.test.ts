...
import {
  velocityToCutoffHz, createVelocityFilter,
  VELOCITY_TIMBRE_MIN_CUTOFF_HZ, VELOCITY_TIMBRE_MAX_CUTOFF_HZ, VELOCITY_TIMBRE_FILTER_Q,
} from './velocityTimbre';

describe('velocityToCutoffHz', () => {
  it('弱い音ほど低く、強い音ほど高い（対数補間・両端は定数）', () => {
    expect(velocityToCutoffHz(0)).toBeCloseTo(VELOCITY_TIMBRE_MIN_CUTOFF_HZ, 6);
    expect(velocityToCutoffHz(1)).toBeCloseTo(VELOCITY_TIMBRE_MAX_CUTOFF_HZ, 6);
    const mid = velocityToCutoffHz(0.5);
    // 対数補間なので中点は幾何平均（約 4.7kHz）
    expect(mid).toBeCloseTo(Math.sqrt(VELOCITY_TIMBRE_MIN_CUTOFF_HZ * VELOCITY_TIMBRE_MAX_CUTOFF_HZ), 3);
    expect(velocityToCutoffHz(0.22)).toBeLessThan(velocityToCutoffHz(0.74));
  });

  it('範囲外・非数は安全側（素通し＝最大）に丸める', () => {
    expect(velocityToCutoffHz(1.5)).toBeCloseTo(VELOCITY_TIMBRE_MAX_CUTOFF_HZ, 6);
